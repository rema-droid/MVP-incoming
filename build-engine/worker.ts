...
/**
 * Security: Safe wrapper for spawning processes without a shell.
 * This mitigates command injection vulnerabilities.
 */
  ...
  return new Promise<{ stdout: string; stderr: string }>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        const error = new Error(`Command failed: ${command} ${args.join(' ')}`) as Error & { stdout: string; stderr: string };
        error.stdout = stdout;
        error.stderr = stderr;
        reject(error);
      }
...
export const worker = new Worker('Run Cloud', async job => {
  ...
  // Security: Sanitize repoId to prevent path traversal risks
  const repoId = String(job.data.repoId || job.data.id || 'unknown').replace(/[^a-zA-Z0-9-]/g, '');
  ...
  try {
    ...
    // preventing argument injection (e.g., if a URL starts with a hyphen).
    ...
    // 4. Cleanup
    // Security: Use fs.rm instead of shell command for safer file removal.
