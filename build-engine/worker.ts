...
/**
 * Securely spawns a child process and returns a promise.
 * Mitigates command injection by using arguments array.
 */
function spawnAsync(command: string, args: string[], options: import('child_process').SpawnOptions = {}): Promise<{ stdout: string; stderr: string }> {
...
export const worker = new Worker('Run Cloud', async job => {
  ...
  if (!isValidRepoUrl(githubUrl)) {
    throw new Error("Invalid repository URL provided to worker");
  }
