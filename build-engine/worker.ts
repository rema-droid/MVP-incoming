...
/**
 * Executes a command with an arguments array to prevent command injection.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        (error as { stderr?: string, stdout?: string }).stderr = stderr;
        (error as { stderr?: string, stdout?: string }).stdout = stdout;
        reject(error);
      }
    });
