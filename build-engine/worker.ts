...
async function run(command: string, args: string[], options: { env?: Record<string, string | undefined>; cwd?: string } = {}) {
  return new Promise<{ stdout: string; stderr: string }>((resolve, reject) => {
    ...
    child.on('close', code => {
      ...
      else reject({ message: `Command failed: ${command} ${args.join(' ')}`, stdout, stderr, code });
    });
...
export const worker = new Worker('Run Cloud', async job => {
  ...
  // Sanitize repoId to prevent command injection in paths/app names
  ...
  try {
    ...
    // 4. Cleanup
    // Use fs/promises instead of rm -rf shell command
