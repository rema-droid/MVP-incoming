...
const REPO_URL_REGEX = /^https:\/\/[a-zA-Z0-9._\-\/@#+:]+$/;
const ID_SANI_REGEX = /[^a-zA-Z0-9-]/g;

/**
 * Executes a command in a child process with shell: false to prevent command injection.
 */
...
export const worker = new Worker('Run Cloud', async job => {
  ...
  // Security: Validate URL to prevent SSRF or command injection
  if (!githubUrl || !REPO_URL_REGEX.test(githubUrl)) {
    console.error(`[Worker] Invalid repository URL: ${githubUrl}`);
    await redis.set(`repo:${repoId}:status`, 'failed');
    await redis.set(`repo:${repoId}:logs`, 'Invalid repository URL format. Must be a secure HTTPS link.');
    throw new Error('Invalid repository URL');
  }

  // Security: Sanitize IDs used in shell commands or file paths
  const safeRepoId = String(repoId).replace(ID_SANI_REGEX, '');
