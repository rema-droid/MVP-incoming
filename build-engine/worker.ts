  ...
  return new Promise<{ stdout: string; stderr: string }>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        const error = new Error(`Command failed with code ${code}`) as Record<string, unknown>;
        error.stdout = stdout;
        error.stderr = stderr;
        reject(error);
      }
...
export const worker = new Worker('Run Cloud', async job => {
  ...
  if (!githubUrl || !GITHUB_URL_REGEX.test(githubUrl)) {
    ...
    await redis.set(`repo:${job.data.repoId || job.data.id || 'unknown'}:status`, 'failed');
    await redis.set(`repo:${job.data.repoId || job.data.id || 'unknown'}:logs`, 'Invalid GitHub URL provided.');
    return;
  }
  ...
  try {
    ...
    try {
      ...
      await runCommand('flyctl', ['apps', 'create', appName, '--machines', '--org', 'personal'], { env: { ...process.env, FLY_API_TOKEN: process.env.FLY_API_TOKEN } });
    } catch {
    ...
    const { stdout, stderr } = await runCommand('flyctl', ['deploy', '.', '--app', appName, '--nixpacks', '--ha=false'], { cwd: tmpDir, env: { ...process.env, FLY_API_TOKEN: process.env.FLY_API_TOKEN } });
    console.log(stdout); 
