...
/**
 * Safe wrapper for executing external commands using child_process.spawn.
 * Avoids shell injection by passing arguments as an array and disabling shell execution.
 */
...
export const worker = new Worker('Run Cloud', async job => {
  ...
  if (!isValidRepoUrl(githubUrl)) {
    ...
    await redis.set(`repo:${repoId}:logs`, `Security Error: Invalid repository URL provided: ${githubUrl}`);
    return;
  }
  ...
  try {
    ...
    // Use -- separator to prevent repo URL from being interpreted as an argument
