...
const APP_NAME_REGEX = /^[a-zA-Z0-9-_]+$/;
...
export const worker = new Worker('Run Cloud', async job => {
  ...
  if (!GITHUB_URL_REGEX.test(githubUrl) || !APP_NAME_REGEX.test(String(repoId))) {
    console.error(`[Worker] Security block: Invalid URL (${githubUrl}) or ID (${repoId})`);
    throw new Error('Security block: Invalid URL or ID');
  }
  ...
  try {
    ...
    const error = rawError as Error & { code: number; stdout: string; stderr: string };
