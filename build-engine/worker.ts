...
async function run(cmd: string, args: string[], options: { env?: Record<string, string | undefined>; cwd?: string } = {}) {
  return new Promise<{ stdout: string; stderr: string }>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      else reject({ message: `Command failed: ${cmd} ${args.join(' ')}`, stdout, stderr });
    });
...
export const worker = new Worker('Run Cloud', async job => {
  ...
  const safeRepoId = repoId.replace(/[^a-zA-Z0-9-]/g, '');
  const appName = `gitmurph-${safeRepoId.toLowerCase()}`;

  if (!githubUrl.match(/^https:\/\/[a-zA-Z0-9._\-\/@#+:]+$/)) {
