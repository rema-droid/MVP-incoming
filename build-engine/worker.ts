...
// Use a strict regex for GitHub URLs to prevent SSRF and command injection.
...
export const worker = new Worker('Run Cloud', async job => {
  ...
  // Security: Validate the GitHub URL
  if (!githubUrl || !GITHUB_URL_REGEX.test(githubUrl)) {
    throw new Error('Invalid or unauthorized GitHub URL provided.');
  }

  // Security: Sanitize repoId to prevent path traversal or other injection risks
  ...
  try {
    ...
    const { stdout, stderr } = await runCommand('flyctl', [
