...
export const worker = new Worker('Run Cloud', async job => {
  ...
  if (!repoUrlRegex.test(githubUrl)) {
    console.error(`[Worker] Invalid repository URL: ${githubUrl}`);
    await redis.set(`repo:${repoId}:status`, 'failed');
    await redis.set(`repo:${repoId}:logs`, 'Invalid repository URL');
    return;
  }

  // Sanitize repoId for use in app name and folder name
  const safeRepoId = String(repoId).replace(/[^a-zA-Z0-9-]/g, '');
