...
function manageCache(cache: Map<string, unknown>) {
...
const SUMMARY_TYPE_RE = {
  website: /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/,
  behind: /(api|server|backend|graphql|microservice|rest)/,
  ...
  game: /(game|play|fun)/,
  media: /(image|video|audio|media|design|editor|creative)/,
};
...
const CAT_RE = {
  ...
  behind: /(api|server|backend)/,
  data: /(data|analytics|chart)/,
  fun: /(game|play)/,
  cli: /(cli|terminal|command)/,
  media: /(image|video|audio|media|design|creative)/,
  safe: /(security|auth|encryption|privacy)/,
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_RE.ai.test(combined)) result = "Smart helper";
  else if (CAT_RE.web.test(combined)) result = "Website";
  else if (CAT_RE.behind.test(combined)) result = "Behind-the-scenes worker";
  else if (CAT_RE.data.test(combined)) result = "Numbers and charts";
  else if (CAT_RE.fun.test(combined)) result = "Fun stuff";
  else if (CAT_RE.cli.test(combined)) result = "Text-based tool";
  else if (CAT_RE.media.test(combined)) result = "Creative tool";
  else if (CAT_RE.safe.test(combined)) result = "Safety and privacy";
  else if (CAT_RE.setup.test(combined)) result = "Setup helper";

  CATEGORY_CACHE.set(cacheKey, result);
  manageCache(CATEGORY_CACHE);
  return result;
}
