...
function getCacheKey(repo: RepoLike): string {
  return JSON.stringify({
    title: repo.title,
    desc: repo.plainEnglishDescription,
    lang: repo.language,
    topics: repo.topics,
  });
}
...
const CAT_WEBSITE = /(react|next|vue|web|website|browser)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  else if (CAT_WEBSITE.test(combined)) result = "Website";
  ...
  else if (CAT_INFRA.test(combined)) result = "Setup helper";
