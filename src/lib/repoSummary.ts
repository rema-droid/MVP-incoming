...
const RE_TYPE_BACKEND = /behind/i;
const RE_TYPE_CLI = /text-based/i;
const RE_TYPE_AI = /smart helper/i;
const RE_TYPE_DATA = /information/i;
const RE_TYPE_CREATIVE = /creative/i;
const RE_TYPE_FUN = /fun/i;

// Simple FIFO cache to avoid repeated expensive computations
...
const RE_FRIENDLY_AI = /(ai|llm|chat|gpt|assistant|agent)/;
const RE_FRIENDLY_WEB = /(react|next|vue|web|website|browser)/;
const RE_FRIENDLY_BACKEND = /(api|server|backend)/;
const RE_FRIENDLY_DATA = /(data|analytics|chart)/;
const RE_FRIENDLY_FUN = /(game|play)/;
const RE_FRIENDLY_CLI = /(cli|terminal|command)/;
const RE_FRIENDLY_CREATIVE = /(image|video|audio|media|design|creative)/;
const RE_FRIENDLY_SECURITY = /(security|auth|encryption|privacy)/;
const RE_FRIENDLY_INFRA = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_FRIENDLY_AI.test(combined)) result = "Smart helper";
  else if (RE_FRIENDLY_WEB.test(combined)) result = "Website";
  else if (RE_FRIENDLY_BACKEND.test(combined)) result = "Behind-the-scenes worker";
  else if (RE_FRIENDLY_DATA.test(combined)) result = "Numbers and charts";
  else if (RE_FRIENDLY_FUN.test(combined)) result = "Fun stuff";
  else if (RE_FRIENDLY_CLI.test(combined)) result = "Text-based tool";
  else if (RE_FRIENDLY_CREATIVE.test(combined)) result = "Creative tool";
  else if (RE_FRIENDLY_SECURITY.test(combined)) result = "Safety and privacy";
  else if (RE_FRIENDLY_INFRA.test(combined)) result = "Setup helper";
