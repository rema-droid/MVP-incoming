...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  setWithEviction(SUMMARY_CACHE, cacheKey, summary);
  return summary;
}
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_RE_AI.test(combined)) label = "Smart helper";
  else if (CAT_RE_WEBSITE.test(combined)) label = "Website";
  else if (CAT_RE_BACKEND.test(combined)) label = "Behind-the-scenes worker";
  else if (CAT_RE_DATA.test(combined)) label = "Numbers and charts";
  else if (CAT_RE_FUN.test(combined)) label = "Fun stuff";
  else if (CAT_RE_CLI.test(combined)) label = "Text-based tool";
  else if (CAT_RE_CREATIVE.test(combined)) label = "Creative tool";
  else if (CAT_RE_SECURITY.test(combined)) label = "Safety and privacy";
  else if (CAT_RE_INFRA.test(combined)) label = "Setup helper";

  setWithEviction(CATEGORY_CACHE, cacheKey, label);
  return label;
}
