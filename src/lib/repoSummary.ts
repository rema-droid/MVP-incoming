...
// Pre-compiled regex for summarizeRepoForBeginners
...
function enforceCacheLimit(cache: Map<unknown, unknown>) {
...
// Pre-compiled regex for friendlyCategoryLabel
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  else if (RE_CAT_API.test(combined)) result = "Behind-the-scenes worker";
