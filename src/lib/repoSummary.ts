...
function getCacheKey(repo: RepoLike): string {
  ...
  return `key:${repo.title}|${repo.plainEnglishDescription}|${repo.language}|${(repo.topics || []).join(",")}`;
}
...
/* ── Category Classification Regexes ── */
...
const RE_FUN = /(game|play|fun)/i;
const RE_CREATIVE = /(image|video|audio|media|design|editor|creative)/i;
const RE_INFRA = /(docker|kubernetes|cloud|devops|infra)/i;
...
const RE_TYPE_INFO = /numbers and charts|information explorer/i;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  manageCache(CATEGORY_CACHE, key, "Community tool");
