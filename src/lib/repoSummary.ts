...
/* ── Hoisted Regexes for Classification ── */
...
/* ── Hoisted Regexes for Category Label ── */
...
const CAT_CLOUD = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_AI.test(combined)) result = "Smart helper";
  else if (CAT_WEB.test(combined)) result = "Website";
  else if (CAT_BACKEND.test(combined)) result = "Behind-the-scenes worker";
  else if (CAT_DATA.test(combined)) result = "Numbers and charts";
  else if (CAT_GAME.test(combined)) result = "Fun stuff";
  else if (CAT_CLI.test(combined)) result = "Text-based tool";
  else if (CAT_CREATIVE.test(combined)) result = "Creative tool";
  else if (CAT_SECURITY.test(combined)) result = "Safety and privacy";
  else if (CAT_CLOUD.test(combined)) result = "Setup helper";
