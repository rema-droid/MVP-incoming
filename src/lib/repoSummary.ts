...
/**
 * ⚡ Bolt: Hoisted constants and regex literals to the module level
 * to avoid expensive re-allocation and re-compilation on every function call.
 */
...
/* ── Bolt: Bounded FIFO caches for O(1) lookups and constant-time eviction ── */
const MAX_CACHE_SIZE = 1000;
const SUMMARY_CACHE = new Map<string, RepoSummary>();
const CATEGORY_CACHE = new Map<string, string>();

/**
 * ⚡ Bolt: Generates a unique cache key for a repository.
 * Uses repo.id if available for O(1) lookups, otherwise constructs a composite key.
 */
...
/**
 * ⚡ Bolt: Manages bounded Map caches with O(1) FIFO eviction.
 */
...
const SUMMARY_RE_MAP = {
  frontend: /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/,
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  manageCache(SUMMARY_CACHE, cacheKey, result);
  return result;
}

const CATEGORY_RE_MAP = {
  ai: /(ai|llm|chat|gpt|assistant|agent)/,
  web: /(react|next|vue|web|website|browser)/,
  ...
  infra: /(docker|kubernetes|cloud|devops|infra)/,
};
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CATEGORY_RE_MAP.ai.test(combined)) result = "Smart helper";
  else if (CATEGORY_RE_MAP.web.test(combined)) result = "Website";
  else if (CATEGORY_RE_MAP.backend.test(combined)) result = "Behind-the-scenes worker";
  else if (CATEGORY_RE_MAP.data.test(combined)) result = "Numbers and charts";
  else if (CATEGORY_RE_MAP.game.test(combined)) result = "Fun stuff";
  else if (CATEGORY_RE_MAP.cli.test(combined)) result = "Text-based tool";
  else if (CATEGORY_RE_MAP.creative.test(combined)) result = "Creative tool";
  else if (CATEGORY_RE_MAP.security.test(combined)) result = "Safety and privacy";
  else if (CATEGORY_RE_MAP.infra.test(combined)) result = "Setup helper";

  manageCache(CATEGORY_CACHE, cacheKey, result);
  return result;
}
