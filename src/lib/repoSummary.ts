...
// Hoisting jargon swaps to a module-level constant avoids re-allocating and re-compiling
// dozens of RegExp objects on every function call.
...
// FIFO caches to prevent redundant processing of repository metadata.
// 500 entries provides a good balance between hit rate and memory usage.
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  const { title = "", plainEnglishDescription: raw = "", topics = [], language = "" } = repo;
  // Sort topics to ensure consistent cache keys regardless of input order
  const cacheKey = `${title}|${raw}|${[...topics].sort().join(",")}|${language}`;

  const cached = summaryCache.get(cacheKey);
  if (cached) {
    // Return a clone to prevent external mutation of the cache entry
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  const { title = "", plainEnglishDescription: raw = "", topics = [] } = repo;
  const cacheKey = `${title}|${raw}|${[...topics].sort().join(",")}`;
