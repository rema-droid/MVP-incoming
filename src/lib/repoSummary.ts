...
/* ── Regular expressions hoisted for performance ── */
...
const RE_WEB_CAT = /(react|next|vue|web|website|browser)/;
...
/* ── Jargon swaps hoisted for performance ── */
...
function stripLangNames(text: string): string {
  return text.replace(RE_LANG, "").replace(RE_MULTI_SPACE, " ").trim();
}
...
function getCacheKey(repo: RepoLike): string {
  // Use a stable key that represents the repo's content used for summarization
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_AI_CAT.test(combined)) result = "Smart helper";
  else if (RE_WEB_CAT.test(combined)) result = "Website";
  else if (RE_BACKEND_CAT.test(combined)) result = "Behind-the-scenes worker";
  else if (RE_DATA_CAT.test(combined)) result = "Numbers and charts";
  else if (RE_GAME_CAT.test(combined)) result = "Fun stuff";
  else if (RE_CLI_CAT.test(combined)) result = "Text-based tool";
  else if (RE_CREATIVE_CAT.test(combined)) result = "Creative tool";
  else if (RE_SECURITY_CAT.test(combined)) result = "Safety and privacy";
  else if (RE_INFRA_CAT.test(combined)) result = "Setup helper";
...
export function buildLongBeginnerStory(repo: RepoLike): LongBeginnerStory {
  ...
  if (topics.length > 0) {
    ...
    if (friendlyTopics.length > 0) {
      p7 = `Some labels the makers gave this project: ${friendlyTopics.join(
        ", "
      )}. Don't worry if those words sound unfamiliar — they are just tags, like hashtags on a social media post. They help other people find this app when searching.`;
    }
  }

  const paragraphs = [p1, p2, p3, p4, p5, p6, p7].filter((p) => p.length > 0);
