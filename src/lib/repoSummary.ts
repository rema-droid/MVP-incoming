...
const LANG_RE = /\b(Python|JavaScript|TypeScript|Rust|Go|Golang|C\+\+|C#|Java|Ruby|PHP|Swift|Kotlin|Scala|Perl|Haskell|Elixir|Dart|Lua|R|Julia|Zig|Nim|OCaml|Clojure|Erlang)\b/gi;
...
const SWAPS: Array<[RegExp, string]> = [
  ...
  [/\bML\b/gi, "machine learning (teaching a computer to learn patterns)"],
  [/\bmachine learning\b/gi, "teaching a computer to learn patterns"],
  [/\bAI\b/gi, "a smart helper that can think and learn"],
...
function getCacheKey(repo: RepoLike): string {
  // Use title and description for cache key as they are the primary inputs
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  setWithEviction(SUMMARY_CACHE, SUMMARY_KEYS, cacheKey, result);
  return result;
}

const CAT_AI = /(ai|llm|chat|gpt|assistant|agent)/;
const CAT_WEB = /(react|next|vue|web|website|browser)/;
const CAT_BACKEND = /(api|server|backend)/;
const CAT_DATA = /(data|analytics|chart)/;
const CAT_FUN = /(game|play)/;
const CAT_CLI = /(cli|terminal|command)/;
const CAT_CREATIVE = /(image|video|audio|media|design|creative)/;
const CAT_SECURITY = /(security|auth|encryption|privacy)/;
const CAT_INFRA = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_AI.test(combined)) label = "Smart helper";
  else if (CAT_WEB.test(combined)) label = "Website";
  else if (CAT_BACKEND.test(combined)) label = "Behind-the-scenes worker";
  else if (CAT_DATA.test(combined)) label = "Numbers and charts";
  else if (CAT_FUN.test(combined)) label = "Fun stuff";
  else if (CAT_CLI.test(combined)) label = "Text-based tool";
  else if (CAT_CREATIVE.test(combined)) label = "Creative tool";
  else if (CAT_SECURITY.test(combined)) label = "Safety and privacy";
  else if (CAT_INFRA.test(combined)) label = "Setup helper";

  setWithEviction(CATEGORY_CACHE, CATEGORY_KEYS, cacheKey, label);
  return label;
}
