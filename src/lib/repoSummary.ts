...
function getSummarizeCacheKey(repo: RepoLike): string {
  // Use id if available, otherwise a composite of key properties
  // We check for undefined explicitly because id could be 0
  const id = (repo as { id?: number | string }).id;
  if (id !== undefined) return `id:${id}`;
  return `t:${repo.title || ""} d:${repo.plainEnglishDescription || ""} l:${repo.language || ""} tp:${(repo.topics || []).join(",")}`;
}
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  addToCache(SUMMARIZE_CACHE, cacheKey, result);
  return result;
}
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (/(ai|llm|chat|gpt|assistant|agent)/.test(combined)) {
    addToCache(CATEGORY_CACHE, cacheKey, "Smart helper");
    return "Smart helper";
  }
  if (/(react|next|vue|web|website|browser)/.test(combined)) {
    addToCache(CATEGORY_CACHE, cacheKey, "Website");
    return "Website";
  }
  if (/(api|server|backend)/.test(combined)) {
    addToCache(CATEGORY_CACHE, cacheKey, "Behind-the-scenes worker");
    return "Behind-the-scenes worker";
  }
  if (/(data|analytics|chart)/.test(combined)) {
    addToCache(CATEGORY_CACHE, cacheKey, "Numbers and charts");
    return "Numbers and charts";
  }
  if (/(game|play)/.test(combined)) {
    addToCache(CATEGORY_CACHE, cacheKey, "Fun stuff");
    return "Fun stuff";
  }
  if (/(cli|terminal|command)/.test(combined)) {
    addToCache(CATEGORY_CACHE, cacheKey, "Text-based tool");
    return "Text-based tool";
  }
  if (/(image|video|audio|media|design|creative)/.test(combined)) {
    addToCache(CATEGORY_CACHE, cacheKey, "Creative tool");
    return "Creative tool";
  }
  if (/(security|auth|encryption|privacy)/.test(combined)) {
    addToCache(CATEGORY_CACHE, cacheKey, "Safety and privacy");
    return "Safety and privacy";
  }
  if (/(docker|kubernetes|cloud|devops|infra)/.test(combined)) {
    addToCache(CATEGORY_CACHE, cacheKey, "Setup helper");
    return "Setup helper";
  }
  addToCache(CATEGORY_CACHE, cacheKey, "Community tool");
