...
const SUMMARY_MATCHERS: Array<[RegExp, string | null, string]> = [
  ...
  [/(python|django|flask|fastapi)/, null, "Popular with people who work with numbers and small handy tools"],
  [/(rust)/, null, "Made to be really fast and really safe — like a sports car with extra seat belts"],
  ...
  [/(auth|login|oauth|jwt|security|encryption)/, null, "Keeps your passwords, accounts, and private stuff safe"],
  [/(docker|container)/, null, "Packages everything neatly so you can start with one tap — no fuss"],
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  for (const [re, hint, pill] of SUMMARY_MATCHERS) {
    if (re.test(combined)) {
  ...
  enforceCacheLimit(summaryCache);
