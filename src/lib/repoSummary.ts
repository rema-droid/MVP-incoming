...
const SUMMARY_MATCHERS = [
  { re: /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/, type: "Website you can visit", pill: "Opens in your browser like any normal website" },
  { re: /(api|server|backend|graphql|microservice|rest)/, type: "Behind-the-scenes helper", pill: "Works quietly in the background so other apps can do their job" },
  { re: /(cli|command tool|terminal|command-line|tool)/, type: "Text-based helper", pill: "You tell it what to do by typing — no clicking needed" },
  { re: /(python|django|flask|fastapi)/, type: null, pill: "Popular with people who work with numbers and small handy tools" },
  { re: /(rust)/, type: null, pill: "Made to be really fast and really safe — like a sports car with extra seat belts" },
  { re: /(data|analytics|pandas|spark|ml|model|dataset)/, type: "Information explorer", pill: "Helps you look at numbers, charts, and patterns to understand things better" },
  { re: /(auth|login|oauth|jwt|security|encryption)/, type: null, pill: "Keeps your passwords, accounts, and private stuff safe" },
  { re: /(docker|container)/, type: null, pill: "Packages everything neatly so you can start with one tap — no fuss" },
  { re: /(ai|llm|gpt|agent|neural|transformer|langchain|chat)/, type: "Smart helper", pill: "Uses a computer brain to answer questions or do tasks for you" },
  { re: /(game|play|fun)/, type: "Fun and games", pill: "Something you play or have fun with — just for the joy of it" },
  { re: /(image|video|audio|media|design|editor|creative)/, type: "Creative tool", pill: "Helps you make, edit, or enjoy pictures, videos, or sounds" },
];
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  const cacheKey = `${repo.title || ""}|${raw}|${topicsText}|${repo.language || ""}`;

  const cached = summaryCache.get(cacheKey);
  if (cached) return { ...cached, goodForPills: [...cached.goodForPills] };

  const combined =
    `${repo.title || ""} ${raw} ${(repo.topics || []).join(" ")} ${repo.language || ""}`.toLowerCase();

  const pills: string[] = [];
  const typeHints: string[] = [];

  for (const matcher of SUMMARY_MATCHERS) {
    if (matcher.re.test(combined)) {
      if (matcher.type) typeHints.push(matcher.type);
      if (matcher.pill) pills.push(matcher.pill);
    }
  }
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  const cacheKey = `${repo.title || ""}|${raw}|${sortedTopics}`;
