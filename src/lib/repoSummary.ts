...
function simplifyWords(text: string): string {
  ...
  // Optimization: use a module-level constant for swaps to avoid re-allocation
...
const CATEGORY_LABEL_LIMIT = 1000;

/* ── Short summary (card-level) ── */
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  // Optimization: memoize based on a composite key of the repo's descriptive fields
  const cacheKey = `${repo.title || ""}|${repo.plainEnglishDescription || ""}|${repo.language || ""}|${(repo.topics || []).join(",")}`;
  ...
  // Simple LRU-like limit
  if (summaryCache.size >= CATEGORY_LABEL_LIMIT) {
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  // Optimization: memoize results for performance in lists
  const cacheKey = `${repo.title || ""}|${repo.plainEnglishDescription || ""}|${repo.language || ""}|${(repo.topics || []).join(",")}`;
  ...
  if (/(ai|llm|chat|gpt|assistant|agent)/.test(combined)) return cacheAndReturn("Smart helper");
  if (/(react|next|vue|web|website|browser)/.test(combined)) return cacheAndReturn("Website");
  if (/(api|server|backend)/.test(combined)) return cacheAndReturn("Behind-the-scenes worker");
  if (/(data|analytics|chart)/.test(combined)) return cacheAndReturn("Numbers and charts");
  if (/(game|play)/.test(combined)) return cacheAndReturn("Fun stuff");
  if (/(cli|terminal|command)/.test(combined)) return cacheAndReturn("Text-based tool");
  if (/(image|video|audio|media|design|creative)/.test(combined)) return cacheAndReturn("Creative tool");
  if (/(security|auth|encryption|privacy)/.test(combined)) return cacheAndReturn("Safety and privacy");
  if (/(docker|kubernetes|cloud|devops|infra)/.test(combined)) return cacheAndReturn("Setup helper");
  return cacheAndReturn("Community tool");

  function cacheAndReturn(label: string): string {
    if (categoryCache.size >= CATEGORY_LABEL_LIMIT) {
