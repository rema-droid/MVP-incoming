...
const SWAPS: Array<[RegExp, string]> = [
  ...
  [/\bkubernetes\b/gi, "a manager that keeps many apps smoothly"],
...
/* ── Simple FIFO Cache for repository summaries ── */
const CACHE_LIMIT = 1000;
const summaryCache = new Map<string, RepoSummary>();

function getCacheKey(repo: RepoLike): string {
  // If we have a stable ID, use it for O(1) cache lookups
  if (repo.id !== undefined) return `id:${repo.id}`;
  // Fallback to a composite key for objects without a stable ID
  return `comp:${repo.title}|${repo.plainEnglishDescription}|${repo.language}|${(repo.topics || []).join(",")}`;
}
...
/* ── Pre-compiled regex for classification ── */
