...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (SUMMARY_MATCHERS.web.test(combined)) {
  ...
  if (SUMMARY_MATCHERS.devops.test(combined)) {
...
const CATEGORY_MATCHERS = {
  ...
  devops: /(docker|kubernetes|cloud|devops|infra)/,
};
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CATEGORY_MATCHERS.web.test(combined)) return "Website";
  ...
  if (CATEGORY_MATCHERS.devops.test(combined)) return "Setup helper";
