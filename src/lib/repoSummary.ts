...
/* ── Categorization Regex Constants ── */
...
const RE_AI_LABEL = /(ai|llm|chat|gpt|assistant|agent)/;
const RE_WEBSITE_LABEL = /(react|next|vue|web|website|browser)/;
const RE_BACKEND_LABEL = /(api|server|backend)/;
const RE_DATA_LABEL = /(data|analytics|chart)/;
const RE_FUN_LABEL = /(game|play)/;
const RE_CLI_LABEL = /(cli|terminal|command)/;
const RE_CREATIVE_LABEL = /(image|video|audio|media|design|creative)/;
const RE_SECURITY_LABEL = /(security|auth|encryption|privacy)/;
const RE_INFRA_LABEL = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_AI_LABEL.test(combined)) result = "Smart helper";
  else if (RE_WEBSITE_LABEL.test(combined)) result = "Website";
  else if (RE_BACKEND_LABEL.test(combined)) result = "Behind-the-scenes worker";
  else if (RE_DATA_LABEL.test(combined)) result = "Numbers and charts";
  else if (RE_FUN_LABEL.test(combined)) result = "Fun stuff";
  else if (RE_CLI_LABEL.test(combined)) result = "Text-based tool";
  else if (RE_CREATIVE_LABEL.test(combined)) result = "Creative tool";
  else if (RE_SECURITY_LABEL.test(combined)) result = "Safety and privacy";
  else if (RE_INFRA_LABEL.test(combined)) result = "Setup helper";
