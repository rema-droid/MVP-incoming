...
function simplifyWords(text: string): string {
  ...
  // Bolt optimization: SWAPS moved outside function to avoid re-allocation
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  // Bolt optimization: Memoize expensive summary logic
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  // Bolt optimization: Memoize category labeling logic
  const cacheKey = `${repo.title}|${repo.plainEnglishDescription}|${repo.language}|${(repo.topics || []).join(",")}`;
  ...
  function cacheAndReturn(label: string) {
