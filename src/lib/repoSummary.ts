...
/* ── PERF: Size-limited caches for expensive summarization ── */
const CACHE_LIMIT = 500;
const SUMMARY_CACHE = new Map<string, RepoSummary>();
const CATEGORY_CACHE = new Map<string, string>();

/**
 * Enforce a size limit on the cache using a simple FIFO eviction policy.
 * Typed as unknown to satisfy ESLint.
 */
...
function getSummarizeCacheKey(repo: RepoLike): string {
  // Use a stable key based on the repo properties.
  // Using title, language, and description should be unique enough for trending repos.
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  enforceCacheLimit(SUMMARY_CACHE);
