...
const summarizeCacheKeys: string[] = [];

function addToCache<T>(cache: Map<string, T>, order: string[], key: string, value: T) {
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  addToCache(summarizeCache, summarizeCacheKeys, cacheKey, result);
  return result;
}
