...
// Hoisted out of simplifyWords to avoid re-allocating ~80 regex objects on every call.
...
// Cache for repository summaries to avoid redundant expensive text processing.
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  // We don't include stars in the cache key because the generated summary text
  // currently only depends on title, description, topics, and language.
  const cacheKey = `${repo.title || ""}|${repo.plainEnglishDescription || ""}|${[...(repo.topics || [])].sort().join(",")}|${repo.language || ""}`;
  const cached = summaryCache.get(cacheKey);
  if (cached) {
    // Return a shallow clone of the goodForPills array to prevent external mutation of the cache.
