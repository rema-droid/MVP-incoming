...
function normalizeText(input: string): string {
  return input.replace(NORMALIZE_RE, " ").trim();
}
...
const DOUBLE_SPACE_RE = /\s{2,}/g;

function stripLangNames(text: string): string {
  return text.replace(LANG_RE, "").replace(DOUBLE_SPACE_RE, " ").trim();
}
...
const NEXT_RE = /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/;
const API_RE = /(api|server|backend|graphql|microservice|rest)/;
const CLI_RE = /(cli|command tool|terminal|command-line|tool)/;
const PYTHON_RE = /(python|django|flask|fastapi)/;
const RUST_RE = /(rust)/;
const DATA_RE = /(data|analytics|pandas|spark|ml|model|dataset)/;
const AUTH_RE = /(auth|login|oauth|jwt|security|encryption)/;
const DOCKER_RE = /(docker|container)/;
const AI_RE = /(ai|llm|gpt|agent|neural|transformer|langchain|chat)/;
const GAME_RE = /(game|play|fun)/;
const MEDIA_RE = /(image|video|audio|media|design|editor|creative)/;

const WEBSITE_SENTENCE_RE = /website/i;
const BEHIND_SENTENCE_RE = /behind/i;
const TEXT_BASED_SENTENCE_RE = /text-based/i;
const SMART_HELPER_SENTENCE_RE = /smart helper/i;
const INFORMATION_SENTENCE_RE = /information/i;
const CREATIVE_SENTENCE_RE = /creative/i;
const FUN_SENTENCE_RE = /fun/i;
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  } else if (CREATIVE_SENTENCE_RE.test(typeLabel)) {
    useSentence =
      "Use it to make or change pictures, videos, sounds, or designs — the fun, creative stuff.";
  } else if (FUN_SENTENCE_RE.test(typeLabel)) {
    useSentence =
...
const CAT_AI_RE = /(ai|llm|chat|gpt|assistant|agent)/;
const CAT_WEB_RE = /(react|next|vue|web|website|browser)/;
const CAT_BACKEND_RE = /(api|server|backend)/;
const CAT_DATA_RE = /(data|analytics|chart)/;
const CAT_GAME_RE = /(game|play)/;
const CAT_CLI_RE = /(cli|terminal|command)/;
const CAT_CREATIVE_RE = /(image|video|audio|media|design|creative)/;
const CAT_SECURITY_RE = /(security|auth|encryption|privacy)/;
const CAT_INFRA_RE = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_AI_RE.test(combined)) return "Smart helper";
  if (CAT_WEB_RE.test(combined)) return "Website";
  if (CAT_BACKEND_RE.test(combined)) return "Behind-the-scenes worker";
  if (CAT_DATA_RE.test(combined)) return "Numbers and charts";
  if (CAT_GAME_RE.test(combined)) return "Fun stuff";
  if (CAT_CLI_RE.test(combined)) return "Text-based tool";
  if (CAT_CREATIVE_RE.test(combined)) return "Creative tool";
  if (CAT_SECURITY_RE.test(combined)) return "Safety and privacy";
  if (CAT_INFRA_RE.test(combined)) return "Setup helper";
