...
/**
 * Hoisted replacement map to avoid re-allocation and re-compilation on every call.
 * This provides a measurable speedup for text-heavy summary processing.
 */
...
/**
 * Reusable matchers for repository summaries.
 * Hoisted to avoid re-compilation of regexes during repeated card rendering.
 */
const SUMMARY_MATCHERS = [
  { re: /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/, hint: "Website you can visit", pill: "Opens in your browser like any normal website" },
  { re: /(api|server|backend|graphql|microservice|rest)/, hint: "Behind-the-scenes helper", pill: "Works quietly in the background so other apps can do their job" },
  { re: /(cli|command tool|terminal|command-line|tool)/, hint: "Text-based helper", pill: "You tell it what to do by typing — no clicking needed" },
  { re: /(python|django|flask|fastapi)/, pill: "Popular with people who work with numbers and small handy tools" },
  { re: /(rust)/, pill: "Made to be really fast and really safe — like a sports car with extra seat belts" },
  { re: /(data|analytics|pandas|spark|ml|model|dataset)/, hint: "Information explorer", pill: "Helps you look at numbers, charts, and patterns to understand things better" },
  { re: /(auth|login|oauth|jwt|security|encryption)/, pill: "Keeps your passwords, accounts, and private stuff safe" },
  { re: /(docker|container)/, pill: "Packages everything neatly so you can start with one tap — no fuss" },
  { re: /(ai|llm|gpt|agent|neural|transformer|langchain|chat)/, hint: "Smart helper", pill: "Uses a computer brain to answer questions or do tasks for you" },
  { re: /(game|play|fun)/, hint: "Fun and games", pill: "Something you play or have fun with — just for the joy of it" },
  { re: /(image|video|audio|media|design|editor|creative)/, hint: "Creative tool", pill: "Helps you make, edit, or enjoy pictures, videos, or sounds" },
];

/**
 * Cache for repository summaries with a 500-entry limit and FIFO eviction.
 * Drastically improves performance when rendering large lists or re-visiting categories.
 */
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  const { title = "", plainEnglishDescription = "", topics = [], language = "" } = repo;
  const cacheKey = `${title}|${plainEnglishDescription}|${[...topics].sort().join(",")}|${language}`;
  const cached = summaryCache.get(cacheKey);
  if (cached) {
    return { ...cached, goodForPills: [...cached.goodForPills] };
  }

  const raw = plainEnglishDescription;
  const topicsText = topics.join(" ");
  const combined = `${title} ${raw} ${topicsText} ${language}`.toLowerCase();

  const pills: string[] = [];
  const typeHints: string[] = [];

  for (const m of SUMMARY_MATCHERS) {
    if (m.re.test(combined)) {
      if (m.hint) typeHints.push(m.hint);
      if (m.pill) pills.push(m.pill);
    }
  }
...
/**
 * Matchers for friendly category labels.
 * Hoisted to prevent redundant allocation during classification.
 */
const CATEGORY_MATCHERS = [
  { re: /(ai|llm|chat|gpt|assistant|agent)/, label: "Smart helper" },
  { re: /(react|next|vue|web|website|browser)/, label: "Website" },
  { re: /(api|server|backend)/, label: "Behind-the-scenes worker" },
  { re: /(data|analytics|chart)/, label: "Numbers and charts" },
  { re: /(game|play)/, label: "Fun stuff" },
  { re: /(cli|terminal|command)/, label: "Text-based tool" },
  { re: /(image|video|audio|media|design|creative)/, label: "Creative tool" },
  { re: /(security|auth|encryption|privacy)/, label: "Safety and privacy" },
  { re: /(docker|kubernetes|cloud|devops|infra)/, label: "Setup helper" },
];

/**
 * Cache for category labels to skip regex matching for known repository metadata.
 */
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  const { title = "", plainEnglishDescription = "", topics = [] } = repo;
  const cacheKey = `${title}|${plainEnglishDescription}|${[...topics].sort().join(",")}`;
  const cached = categoryCache.get(cacheKey);
  if (cached) return cached;

  const raw = plainEnglishDescription;
  const topicsText = topics.join(" ");
  const combined = `${title} ${raw} ${topicsText}`.toLowerCase();
