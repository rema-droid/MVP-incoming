...
/* Pre-compiled regex patterns for categorization to avoid allocation/compilation overhead */
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  const summaryResult: RepoSummary = {
    typeLabel,
    short,
    deep,
    goodForPills,
  ...
  manageCache(summaryCache, cacheKey, summaryResult);
  return summaryResult;
}

/* Category compilation regexes */
const RE_CAT_AI = /(ai|llm|chat|gpt|assistant|agent)/;
const RE_CAT_WEB = /(react|next|vue|web|website|browser)/;
const RE_CAT_API = /(api|server|backend)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  else if (RE_CAT_WEB.test(combined)) label = "Website";
  else if (RE_CAT_API.test(combined)) label = "Behind-the-scenes worker";
