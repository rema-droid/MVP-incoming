...
/* ── Hoisted Regex Patterns for summarizing ── */
...
const RE_AUTH = /(auth|login|oauth|jwt|security|encryption)/;
...
/* ── Memoization Caches ── */
const SUMMARY_CACHE = new Map<string | number, RepoSummary>();
const CATEGORY_CACHE = new Map<string | number, string>();
const MAX_CACHE_SIZE = 1000;

function manageCache<T>(cache: Map<string | number, T>, key: string | number, value: T) {
  if (!cache.has(key)) {
...
function getCacheKey(repo: RepoLike): string | number | null {
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (cacheKey !== null) {
    manageCache(SUMMARY_CACHE, cacheKey, summary);
  }

  return summary;
}

/* ── Hoisted Regex Patterns for category labeling ── */
const RE_AI_FRIENDLY = /(ai|llm|chat|gpt|assistant|agent)/;
const RE_WEBSITE_FRIENDLY = /(react|next|vue|web|website|browser)/;
const RE_BACKEND_FRIENDLY = /(api|server|backend)/;
const RE_DATA_FRIENDLY = /(data|analytics|chart)/;
const RE_GAME_FRIENDLY = /(game|play)/;
const RE_CLI_FRIENDLY = /(cli|terminal|command)/;
const RE_CREATIVE_FRIENDLY = /(image|video|audio|media|design|creative)/;
const RE_SECURITY_FRIENDLY = /(security|auth|encryption|privacy)/;
const RE_DOCKER_FRIENDLY = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_AI_FRIENDLY.test(combined)) label = "Smart helper";
  else if (RE_WEBSITE_FRIENDLY.test(combined)) label = "Website";
  else if (RE_BACKEND_FRIENDLY.test(combined)) label = "Behind-the-scenes worker";
  else if (RE_DATA_FRIENDLY.test(combined)) label = "Numbers and charts";
  else if (RE_GAME_FRIENDLY.test(combined)) label = "Fun stuff";
  else if (RE_CLI_FRIENDLY.test(combined)) label = "Text-based tool";
  else if (RE_CREATIVE_FRIENDLY.test(combined)) label = "Creative tool";
  else if (RE_SECURITY_FRIENDLY.test(combined)) label = "Safety and privacy";
  else if (RE_DOCKER_FRIENDLY.test(combined)) label = "Setup helper";
