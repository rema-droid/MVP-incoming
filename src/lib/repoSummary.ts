...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  // Simple cache key based on stable repo fields
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_RE_SECURITY.test(combined)) return "Safety and privacy";
  if (CAT_RE_INFRA.test(combined)) return "Setup helper";
