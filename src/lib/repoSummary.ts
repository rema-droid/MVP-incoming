...
function simplifyWords(text: string): string {
  ...
  // PERF: JARGON_SWAPS is hoisted to module level to avoid redundant regex compilation
...
function getSummarizeCacheKey(repo: RepoLike): string {
  ...
  return `${repo.title || ""}-${repo.language || ""}-${(repo.topics || []).join(",")}-${repo.plainEnglishDescription || ""}-${repo.stars || 0}`;
}

function addToCache<K, V>(cache: Map<K, V>, key: K, value: V) {
  if (cache.size >= FIFO_CACHE_LIMIT) {
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (cached) {
    // Return shallow clone to prevent cache poisoning via goodForPills mutation
  ...
  addToCache(SUMMARIZE_CACHE, key, result);
  // Return shallow clone to prevent cache poisoning via goodForPills mutation
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  addToCache(CATEGORY_CACHE, key, result);
  return result;
}
