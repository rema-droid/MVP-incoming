...
/* ── Pre-compiled jargon replacements to avoid allocation/compilation overhead ── */
...
/* ── Replace jargon with normal words using hoisted SWAPS constant ── */
...
/* ── Classification regexes to avoid re-compilation ── */
const RE_WEBSITE = /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/i;
const RE_BACKEND = /(api|server|backend|graphql|microservice|rest)/i;
const RE_CLI = /(cli|command tool|terminal|command-line|tool)/i;
const RE_PYTHON = /(python|django|flask|fastapi)/i;
const RE_RUST = /(rust)/i;
const RE_DATA = /(data|analytics|pandas|spark|ml|model|dataset)/i;
const RE_SECURITY = /(auth|login|oauth|jwt|security|encryption)/i;
const RE_DOCKER = /(docker|container)/i;
const RE_AI = /(ai|llm|gpt|agent|neural|transformer|langchain|chat)/i;
const RE_GAME = /(game|play|fun)/i;
const RE_CREATIVE = /(image|video|audio|media|design|editor|creative)/i;

const RE_CAT_AI = /(ai|llm|chat|gpt|assistant|agent)/i;
const RE_CAT_WEBSITE = /(react|next|vue|web|website|browser)/i;
const RE_CAT_BACKEND = /(api|server|backend)/i;
const RE_CAT_DATA = /(data|analytics|chart)/i;
const RE_CAT_GAME = /(game|play)/i;
const RE_CAT_CLI = /(cli|terminal|command)/i;
const RE_CAT_CREATIVE = /(image|video|audio|media|design|creative)/i;
const RE_CAT_SECURITY = /(security|auth|encryption|privacy)/i;
const RE_CAT_DOCKER = /(docker|kubernetes|cloud|devops|infra)/i;
...
function manageCache<K, V>(cache: Map<K, V>, key: K, value: V) {
  if (!cache.has(key)) {
...
/* ── Short summary (card-level) with FIFO cache ── */
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (pills.length === 0) {
    pills.push("A free tool anyone can try — made by people who share their work with the world");
  }
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  else if (RE_CAT_WEBSITE.test(combined)) result = "Website";
  ...
  else if (RE_CAT_GAME.test(combined)) result = "Fun stuff";
  else if (RE_CAT_CLI.test(combined)) result = "Text-based tool";
  else if (RE_CAT_CREATIVE.test(combined)) result = "Creative tool";
  else if (RE_CAT_SECURITY.test(combined)) result = "Safety and privacy";
  else if (RE_CAT_DOCKER.test(combined)) result = "Setup helper";

  manageCache(categoryCache, cacheKey, result);
  return result;
}
