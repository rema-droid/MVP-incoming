...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (/(ai|llm|chat|gpt|assistant|agent)/.test(combined)) return cacheCategory(cacheKey, "Smart helper");
  if (/(react|next|vue|web|website|browser)/.test(combined)) return cacheCategory(cacheKey, "Website");
  if (/(api|server|backend)/.test(combined)) return cacheCategory(cacheKey, "Behind-the-scenes worker");
  if (/(data|analytics|chart)/.test(combined)) return cacheCategory(cacheKey, "Numbers and charts");
  if (/(game|play)/.test(combined)) return cacheCategory(cacheKey, "Fun stuff");
  if (/(cli|terminal|command)/.test(combined)) return cacheCategory(cacheKey, "Text-based tool");
  if (/(image|video|audio|media|design|creative)/.test(combined)) return cacheCategory(cacheKey, "Creative tool");
  if (/(security|auth|encryption|privacy)/.test(combined)) return cacheCategory(cacheKey, "Safety and privacy");
  if (/(docker|kubernetes|cloud|devops|infra)/.test(combined)) return cacheCategory(cacheKey, "Setup helper");
  return cacheCategory(cacheKey, "Community tool");
}

function cacheCategory(key: string, label: string): string {
