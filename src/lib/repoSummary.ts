...
const TYPE_RULES: Array<{ re: RegExp; hint?: string; pill?: string }> = [
  {
    re: /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/,
    hint: "Website you can visit",
    pill: "Opens in your browser like any normal website",
  },
  {
    re: /(api|server|backend|graphql|microservice|rest)/,
    hint: "Behind-the-scenes helper",
    pill: "Works quietly in the background so other apps can do their job",
  },
  {
    re: /(cli|command tool|terminal|command-line|tool)/,
    hint: "Text-based helper",
    pill: "You tell it what to do by typing — no clicking needed",
  },
  {
    re: /(python|django|flask|fastapi)/,
    pill: "Popular with people who work with numbers and small handy tools",
  },
  {
    re: /(rust)/,
    pill: "Made to be really fast and really safe — like a sports car with extra seat belts",
  },
  {
    re: /(data|analytics|pandas|spark|ml|model|dataset)/,
    hint: "Information explorer",
    pill: "Helps you look at numbers, charts, and patterns to understand things better",
  },
  {
    re: /(auth|login|oauth|jwt|security|encryption)/,
    pill: "Keeps your passwords, accounts, and private stuff safe",
  },
  {
    re: /(docker|container)/,
    pill: "Packages everything neatly so you can start with one tap — no fuss",
  },
  {
    re: /(ai|llm|gpt|agent|neural|transformer|langchain|chat)/,
    hint: "Smart helper",
    pill: "Uses a computer brain to answer questions or do tasks for you",
  },
  {
    re: /(game|play|fun)/,
    hint: "Fun and games",
    pill: "Something you play or have fun with — just for the joy of it",
  },
  {
    re: /(image|video|audio|media|design|editor|creative)/,
    hint: "Creative tool",
    pill: "Helps you make, edit, or enjoy pictures, videos, or sounds",
  },
];
...
function getCacheKey(repo: RepoLike): string {
  ...
  // Fallback to a composite key if no ID
  return `${repo.title}|${repo.language}|${repo.topics?.join(",")}|${repo.plainEnglishDescription?.slice(0, 100)}`;
}
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  for (const rule of TYPE_RULES) {
    if (rule.re.test(combined)) {
      if (rule.hint) typeHints.push(rule.hint);
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  manageCache(CATEGORY_CACHE, cacheKey, label);
  return label;
}
