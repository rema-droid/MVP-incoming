...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  /* Cache the result (FIFO eviction) */
  if (SUMMARIZE_CACHE.size >= CACHE_LIMIT) {
...
const RE_SMART_HELPER_CAT = /(ai|llm|chat|gpt|assistant|agent)/;
const RE_WEBSITE_CAT = /(react|next|vue|web|website|browser)/;
const RE_BEHIND_SCENES_CAT = /(api|server|backend)/;
const RE_NUMBERS_CAT = /(data|analytics|chart)/;
const RE_FUN_CAT = /(game|play)/;
const RE_TEXT_TOOL_CAT = /(cli|terminal|command)/;
const RE_CREATIVE_TOOL_CAT = /(image|video|audio|media|design|creative)/;
const RE_SAFETY_CAT = /(security|auth|encryption|privacy)/;
const RE_SETUP_CAT = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_SMART_HELPER_CAT.test(combined)) return "Smart helper";
  if (RE_WEBSITE_CAT.test(combined)) return "Website";
  if (RE_BEHIND_SCENES_CAT.test(combined)) return "Behind-the-scenes worker";
  if (RE_NUMBERS_CAT.test(combined)) return "Numbers and charts";
  if (RE_FUN_CAT.test(combined)) return "Fun stuff";
  if (RE_TEXT_TOOL_CAT.test(combined)) return "Text-based tool";
  if (RE_CREATIVE_TOOL_CAT.test(combined)) return "Creative tool";
  if (RE_SAFETY_CAT.test(combined)) return "Safety and privacy";
  if (RE_SETUP_CAT.test(combined)) return "Setup helper";
