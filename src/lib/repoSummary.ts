...
const SWAPS: Array<[RegExp, string]> = [
  ...
  [/\bmicroservice\b/gi, "small separate apps that work together like a team"],
  ...
  [/\bdebugging\b/gi, "finding and fixing mistakes"],
  [/\bdebug\b/gi, "finding and fixing mistakes"],
...
/* ── Classification regex constants ── */
...
function getCacheKey(repo: RepoLike): string {
  ...
  return `${repo.title || ""}:${raw}:${topicsText}:${repo.language || ""}`;
}
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_CAT_DOCKER.test(combined)) return "Setup helper";
