...
function enforceCacheLimit<K, V>(cache: Map<K, V>, limit: number) {
  if (cache.size >= limit) {
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  enforceCacheLimit(summaryCache, CACHE_LIMIT);
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CATEGORY_MATCHERS.ai.test(combined)) return "Smart helper";
  if (CATEGORY_MATCHERS.website.test(combined)) return "Website";
  if (CATEGORY_MATCHERS.backend.test(combined)) return "Behind-the-scenes worker";
  if (CATEGORY_MATCHERS.data.test(combined)) return "Numbers and charts";
  if (CATEGORY_MATCHERS.fun.test(combined)) return "Fun stuff";
  if (CATEGORY_MATCHERS.cli.test(combined)) return "Text-based tool";
  if (CATEGORY_MATCHERS.creative.test(combined)) return "Creative tool";
  if (CATEGORY_MATCHERS.security.test(combined)) return "Safety and privacy";
  if (CATEGORY_MATCHERS.infra.test(combined)) return "Setup helper";
