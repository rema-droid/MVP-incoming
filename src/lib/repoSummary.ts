...
/* ── Pre-compiled jargon swaps to minimize per-call overhead ── */
...
/* ── Hoisted regexes for repository classification to avoid re-allocation and re-compilation ── */
...
function getCacheKey(repo: RepoLike): string {
  ...
  return `${repo.title || ""}|${repo.language || ""}|${repo.plainEnglishDescription || ""}|${(repo.topics || []).join(",")}`;
}
...
/* ── Hoisted category matching regexes ── */
...
const RE_CAT_DOCKER = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_CAT_AI.test(combined)) label = "Smart helper";
  else if (RE_CAT_WEBSITE.test(combined)) label = "Website";
  else if (RE_CAT_BACKEND.test(combined)) label = "Behind-the-scenes worker";
  else if (RE_CAT_DATA.test(combined)) label = "Numbers and charts";
  else if (RE_CAT_GAME.test(combined)) label = "Fun stuff";
  else if (RE_CAT_CLI.test(combined)) label = "Text-based tool";
  else if (RE_CAT_CREATIVE.test(combined)) label = "Creative tool";
  else if (RE_CAT_SECURITY.test(combined)) label = "Safety and privacy";
  else if (RE_CAT_DOCKER.test(combined)) label = "Setup helper";

  manageCache(categoryCache, cacheKey, label);
  return label;
}
