...
function enforceCacheLimit(cache: Map<string, unknown>) {
  if (cache.size >= CACHE_LIMIT) {
    ...
    if (firstKey !== undefined) cache.delete(firstKey as string);
  }
}
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  else if (RE_CAT_INFRA.test(combined)) label = "Setup helper";

  enforceCacheLimit(CATEGORY_CACHE);
  CATEGORY_CACHE.set(cacheKey, label);
  return label;
}
