...
// PERF: Hoisted to module level to avoid repeated allocation
const LANG_RE =
...
// PERF: Size-limited memoization for expensive computations
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (cached) {
    // Return a shallow clone to prevent cache poisoning if callers mutate arrays
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  const cacheKey = getSummarizeCacheKey(repo);
  const cached = categoryCache.get(cacheKey) as string | undefined;
