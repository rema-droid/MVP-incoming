...
/** Robust cache key to avoid collisions or stale entries */
function getSummarizeCacheKey(repo: RepoLike): string {
  ...
  return `title:${repo.title || ""}||lang:${repo.language || ""}||topics:${topics}||desc:${repo.plainEnglishDescription || ""}||stars:${repo.stars || 0}`;
}

const CACHE_LIMIT = 500;
/** FIFO eviction for Map caches */
...
// PERF: Module-level caches for expensive repository functions
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  enforceCacheLimit(SUMMARIZE_CACHE as Map<string, unknown>);
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  enforceCacheLimit(CATEGORY_CACHE as Map<string, unknown>);
