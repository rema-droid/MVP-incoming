...
// PERF: Module-level constants for better performance
const CACHE_LIMIT = 500;
const SUMMARIZE_CACHE = new Map<string, unknown>();
const CATEGORY_CACHE = new Map<string, unknown>();
...
/** Robust composite key for caching repo summaries */
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  const cached = SUMMARIZE_CACHE.get(key) as RepoSummary | undefined;
  if (cached) {
    // Return a shallow clone to prevent cache poisoning
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  const key = getSummarizeCacheKey(repo);
  const cached = CATEGORY_CACHE.get(key) as string | undefined;
