...
const RE_HINT_WEBSITE = /website/i;
const RE_HINT_BEHIND = /behind/i;
const RE_HINT_TEXT = /text-based/i;
const RE_HINT_SMART = /smart helper/i;
const RE_HINT_INFO = /information/i;
const RE_HINT_CREATIVE = /creative/i;
const RE_HINT_FUN = /fun/i;
...
function getSummarizeCacheKey(repo: RepoLike): string {
  ...
  return `title:${repo.title || ""}||desc:${repo.plainEnglishDescription || ""}||lang:${repo.language || ""}||topics:${(repo.topics || []).join(",")}`;
}

/** Evicts oldest item if cache limit reached */
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (cached) {
    // Return a shallow clone to prevent external mutation of the cached array
  ...
  const result = {
    typeLabel,
    short,
    deep: `What is it, in plain words:\n${short}\n\nWho would like this:\n${goodForPills.slice(0, 3).map((p) => `• ${p}`).join("\n") || "• A free tool anyone can try"}\n\nHow to try it:\n${useSentence}`,
    goodForPills,
  };
