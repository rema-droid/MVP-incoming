...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  const key = `${repo.title || ""}|${repo.plainEnglishDescription || ""}|${repo.language || ""}|${(repo.topics || []).join(",")}`.toLowerCase();
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  const key = `${repo.title || ""}|${repo.plainEnglishDescription || ""}|${(repo.topics || []).join(",")}`.toLowerCase();
