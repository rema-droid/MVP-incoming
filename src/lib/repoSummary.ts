...
// PERF: Hoisted to avoid re-allocation on every call
...
// PERF: Size-limited caches to prevent redundant processing
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (cached) {
    // Return a shallow clone to prevent external mutation of the cached object
