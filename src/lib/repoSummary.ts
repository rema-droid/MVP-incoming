...
/* ── Pre-compiled Swaps for simplifyWords ── */
...
/* ── Hoisted Regex Constants for summarizeRepoForBeginners ── */
...
function getCacheKey(repo: RepoLike): string {
  ...
  return `text:${repo.title || ""}-${repo.plainEnglishDescription || ""}-${(repo.topics || []).join(",")}-${repo.language || ""}`;
}

function manageCache(key: string, value: RepoSummary): void {
  const isNew = !SUMMARY_CACHE.has(key);
  SUMMARY_CACHE.set(key, value);
  if (isNew && SUMMARY_CACHE.size > CACHE_LIMIT) {
...
/* ── Hoisted Regex Constants for friendlyCategoryLabel ── */
...
const RE_CAT_CLOUD = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_CAT_CLOUD.test(combined)) return "Setup helper";
