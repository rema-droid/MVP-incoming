...
/**
 * Manages FIFO cache eviction for the given Map.
 */
...
// Hoisted constants to avoid overhead on every call
...
const SUMMARY_TYPE_RULES: Array<[RegExp, string, string]> = [
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  for (const [re, typeHint, pill] of SUMMARY_TYPE_RULES) {
    if (re.test(combined)) {
      if (typeHint) typeHints.push(typeHint);
