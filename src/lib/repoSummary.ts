...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CATEGORY_RE.smart.test(combined)) return "Smart helper";
  if (CATEGORY_RE.website.test(combined)) return "Website";
  if (CATEGORY_RE.backend.test(combined)) return "Behind-the-scenes worker";
  if (CATEGORY_RE.data.test(combined)) return "Numbers and charts";
  if (CATEGORY_RE.fun.test(combined)) return "Fun stuff";
  if (CATEGORY_RE.text.test(combined)) return "Text-based tool";
  if (CATEGORY_RE.creative.test(combined)) return "Creative tool";
  if (CATEGORY_RE.safety.test(combined)) return "Safety and privacy";
  if (CATEGORY_RE.setup.test(combined)) return "Setup helper";
