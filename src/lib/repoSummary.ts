...
function simplifyWords(text: string): string {
  ...
  // Hoisted SWAPS to module level to avoid re-allocation on every call
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  const key = `${repo.title}|${repo.plainEnglishDescription}|${repo.language}|${(repo.topics || []).join(",")}`;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  const key = `${repo.title}|${repo.plainEnglishDescription}|${(repo.topics || []).join(",")}`;
