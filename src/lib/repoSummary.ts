...
import { addToCache } from "./cacheUtils";
...
function getCacheKey(repo: RepoLike): string {
  ...
  return `${repo.title || ""}-${repo.language || ""}-${(repo.topics || []).join(",")}`;
}
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (/(ai|llm|chat|gpt|assistant|agent)/.test(combined)) {
    const label = "Smart helper";
    addToCache(CATEGORY_CACHE, cacheKey, label);
    return label;
  }
