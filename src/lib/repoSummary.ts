...
const SWAPS_KEYS_SORTED = Object.keys(SWAPS_MAP).sort((a, b) => b.length - a.length);
const ESCAPED_KEYS = SWAPS_KEYS_SORTED.map(k => k.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&")).join("|");
const SWAPS_REGEXP = new RegExp(`\\b(${ESCAPED_KEYS})\\b`, "gi");

function simplifyWords(text: string): string {
  const t = normalizeText(text);
  return t.replace(SWAPS_REGEXP, (matched) => {
    const lower = matched.toLowerCase();
    if (lower === "ai" && matched !== "AI") return matched;
    if (lower === "ml" && matched !== "ML") return matched;
...
function manageCache() {
  if (summaryCache.size > CACHE_CAPACITY) {
