...
const SUMMARY_REGEXES = {
  web: /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/,
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CATEGORY_REGEXES.ai.test(combined)) result = "Smart helper";
  else if (CATEGORY_REGEXES.web.test(combined)) result = "Website";
  else if (CATEGORY_REGEXES.backend.test(combined)) result = "Behind-the-scenes worker";
  else if (CATEGORY_REGEXES.data.test(combined)) result = "Numbers and charts";
  else if (CATEGORY_REGEXES.game.test(combined)) result = "Fun stuff";
  else if (CATEGORY_REGEXES.cli.test(combined)) result = "Text-based tool";
  else if (CATEGORY_REGEXES.creative.test(combined)) result = "Creative tool";
  else if (CATEGORY_REGEXES.security.test(combined)) result = "Safety and privacy";
  else if (CATEGORY_REGEXES.setup.test(combined)) result = "Setup helper";
