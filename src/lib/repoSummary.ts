...
const NEXT_REACT_RE = /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/;
const API_SERVER_RE = /(api|server|backend|graphql|microservice|rest)/;
const CLI_TOOL_RE = /(cli|command tool|terminal|command-line|tool)/;
...
const CREATIVE_RE = /(image|video|audio|media|design|editor|creative)/;

/* ── Short summary (card-level) ── */
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  const cacheKey = `${repo.title || ""}|${repo.language || ""}|${repo.plainEnglishDescription || ""}|${(repo.topics || []).join(",")}`;
  ...
  const result = {
    typeLabel,
    short,
    deep: "", // placeholder to be filled below
    goodForPills,
  };
  ...
  setWithEviction(summaryCache, summaryCacheKeys, cacheKey, result);
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_AI_RE.test(combined)) result = "Smart helper";
  else if (CAT_WEB_RE.test(combined)) result = "Website";
  else if (CAT_BACKEND_RE.test(combined)) result = "Behind-the-scenes worker";
  else if (CAT_DATA_RE.test(combined)) result = "Numbers and charts";
  else if (CAT_GAME_RE.test(combined)) result = "Fun stuff";
  else if (CAT_CLI_RE.test(combined)) result = "Text-based tool";
  else if (CAT_CREATIVE_RE.test(combined)) result = "Creative tool";
  else if (CAT_SECURITY_RE.test(combined)) result = "Safety and privacy";
  else if (CAT_INFRA_RE.test(combined)) result = "Setup helper";

  setWithEviction(categoryCache, categoryCacheKeys, cacheKey, result);
  return result;
}
