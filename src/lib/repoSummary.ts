...
const RE_INFRA = /(docker|container)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_RE_AI.test(combined)) {
    const res = "Smart helper";
    manageCache(CATEGORY_CACHE, key, res);
    return res;
  }
  if (CAT_RE_WEBSITE.test(combined)) {
    const res = "Website";
    manageCache(CATEGORY_CACHE, key, res);
    return res;
  }
  if (CAT_RE_BACKEND.test(combined)) {
    const res = "Behind-the-scenes worker";
    manageCache(CATEGORY_CACHE, key, res);
    return res;
  }
  if (CAT_RE_DATA.test(combined)) {
    const res = "Numbers and charts";
    manageCache(CATEGORY_CACHE, key, res);
    return res;
  }
  if (CAT_RE_GAMES.test(combined)) {
    const res = "Fun stuff";
    manageCache(CATEGORY_CACHE, key, res);
    return res;
  }
  if (CAT_RE_CLI.test(combined)) {
    const res = "Text-based tool";
    manageCache(CATEGORY_CACHE, key, res);
    return res;
  }
  if (CAT_RE_CREATIVE.test(combined)) {
    const res = "Creative tool";
    manageCache(CATEGORY_CACHE, key, res);
    return res;
  }
  if (CAT_RE_SECURITY.test(combined)) {
    manageCache(CATEGORY_CACHE, key, "Safety and privacy");
    return "Safety and privacy";
  }
  if (CAT_RE_INFRA.test(combined)) {
    manageCache(CATEGORY_CACHE, key, "Setup helper");
    return "Setup helper";
  }

  const result = "Community tool";
  manageCache(CATEGORY_CACHE, key, result);
  return result;
}
