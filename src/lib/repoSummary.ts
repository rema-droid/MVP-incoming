...
const CASE_SENSITIVE_MAP: Record<string, string> = {
...
const escapedKeys = Object.keys(SWAPS_MAP)
  .sort((a, b) => b.length - a.length)
  .map(escapeRegExp);

const RE_CI = new RegExp(`\\b(${escapedKeys.join("|")})\\b`, "gi");

const escapedCSKeys = Object.keys(CASE_SENSITIVE_MAP)
  .sort((a, b) => b.length - a.length)
  .map(escapeRegExp);

const RE_CS = new RegExp(`\\b(${escapedCSKeys.join("|")})\\b`, "g");

function simplifyWords(text: string): string {
  let t = normalizeText(text);

  // Case-sensitive replacements (e.g. AI, ML)
  t = t.replace(RE_CS, (match) => {
    return CASE_SENSITIVE_MAP[match] || match;
  });

  // Case-insensitive replacements
  t = t.replace(RE_CI, (match) => {
    const lowerMatch = match.toLowerCase();
    return SWAPS_MAP[lowerMatch] || match;
  });
...
/* ── Hoisted Regex constants for repository classification ── */
...
/* ── O(1) FIFO Cache for summarization results ── */
const SUMMARY_CACHE = new Map<string | number, RepoSummary>();
const CACHE_LIMIT = 500;

function getCacheKey(repo: RepoLike): string | number | null {
  if (repo.id !== undefined && repo.id !== null && repo.id !== "") {
    return repo.id;
  }
  return null;
}

function manageCache(key: string | number, value: RepoSummary) {
  if (!SUMMARY_CACHE.has(key) && SUMMARY_CACHE.size >= CACHE_LIMIT) {
    const oldestKey = SUMMARY_CACHE.keys().next().value;
    if (oldestKey !== undefined) {
      SUMMARY_CACHE.delete(oldestKey);
    }
  }
  SUMMARY_CACHE.set(key, value);
}
...
const RE_CAT_WEBSITE = /(react|next|vue|web|website|browser)/;
const RE_CAT_BACKEND = /(api|server|backend)/;
const RE_CAT_DATA = /(data|analytics|chart)/;
const RE_CAT_GAME = /(game|play)/;
