...
/* ── Pre-compiled swaps mapping to avoid compile and allocation overhead on every call ── */
...
/* ── Hoisted regexes to avoid re-allocation and re-compilation overhead during calls ── */
...
function getCacheKey(repo: RepoLike): string | null {
  if (repo.id !== undefined && repo.id !== null) {
    return String(repo.id);
  }
  if (repo.title || repo.plainEnglishDescription) {
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  const result = rawSummarizeRepoForBeginners(repo);

  if (key !== null) {
    manageCache(key, result);
  }
  return result;
}

function rawSummarizeRepoForBeginners(repo: RepoLike): RepoSummary {
