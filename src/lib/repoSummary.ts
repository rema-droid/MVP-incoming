 ...
 * Hoisted jargon swaps to module-level constant to avoid re-allocation and
 * regex re-compilation on every call to simplifyWords.
 */
...
/**
 * PERFORMANCE OPTIMIZATION (Bolt):
 * Hoisted category mappings to module-level constants to avoid redundant allocations.
 */
const CATEGORY_RE_MAP: Array<[RegExp, string, string?]> = [
  [/(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/, "Website you can visit", "Opens in your browser like any normal website"],
  [/(api|server|backend|graphql|microservice|rest)/, "Behind-the-scenes helper", "Works quietly in the background so other apps can do their job"],
  [/(cli|command tool|terminal|command-line|tool)/, "Text-based helper", "You tell it what to do by typing — no clicking needed"],
  [/(python|django|flask|fastapi)/, "", "Popular with people who work with numbers and small handy tools"],
  [/(rust)/, "", "Made to be really fast and really safe — like a sports car with extra seat belts"],
  [/(data|analytics|pandas|spark|ml|model|dataset)/, "Information explorer", "Helps you look at numbers, charts, and patterns to understand things better"],
  [/(auth|login|oauth|jwt|security|encryption)/, "", "Keeps your passwords, accounts, and private stuff safe"],
  [/(docker|container)/, "", "Packages everything neatly so you can start with one tap — no fuss"],
  [/(ai|llm|gpt|agent|neural|transformer|langchain|chat)/, "Smart helper", "Uses a computer brain to answer questions or do tasks for you"],
  [/(game|play|fun)/, "Fun and games", "Something you play or have fun with — just for the joy of it"],
  [/(image|video|audio|media|design|editor|creative)/, "Creative tool", "Helps you make, edit, or enjoy pictures, videos, or sounds"],
];

const FRIENDLY_CATEGORY_RE_MAP: Array<[RegExp, string]> = [
  [/(ai|llm|chat|gpt|assistant|agent)/, "Smart helper"],
  [/(react|next|vue|web|website|browser)/, "Website"],
  [/(api|server|backend)/, "Behind-the-scenes worker"],
  [/(data|analytics|chart)/, "Numbers and charts"],
  [/(game|play)/, "Fun stuff"],
  [/(cli|terminal|command)/, "Text-based tool"],
  [/(image|video|audio|media|design|creative)/, "Creative tool"],
  [/(security|auth|encryption|privacy)/, "Safety and privacy"],
  [/(docker|kubernetes|cloud|devops|infra)/, "Setup helper"],
];

/**
 * PERFORMANCE OPTIMIZATION (Bolt):
 * Implemented a FIFO cache for expensive summary and category calculations.
 * This significantly reduces CPU usage for frequently viewed or listed repositories.
 */
...
function getRepoCacheKey(repo: RepoLike): string {
  ...
  return `title:${repo.title || ""}||lang:${repo.language || ""}||topics:${(repo.topics || []).join(",")}||desc:${repo.plainEnglishDescription || ""}`;
}
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (cached) return { ...cached, goodForPills: [...cached.goodForPills] };
  ...
  for (const [re, hint, pill] of CATEGORY_RE_MAP) {
    if (re.test(combined)) {
      if (hint) typeHints.push(hint);
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  for (const [re, l] of FRIENDLY_CATEGORY_RE_MAP) {
    if (re.test(combined)) {
      label = l;
      break;
    }
  ...
  enforceCacheLimit(categoryCache);
