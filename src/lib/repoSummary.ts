...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  // Use a stable cache key based on properties that affect the summary.
  // Note: Star counts and forks are explicitly excluded to avoid cache misses
  // since the summary logic is independent of popularity metrics.
  const title = repo.title || "";
  const plainEnglishDescription = repo.plainEnglishDescription || "";
  const topicsStr = [...(repo.topics || [])].sort().join(",");
  const language = repo.language || "";
  const cacheKey = `${title}|${plainEnglishDescription}|${topicsStr}|${language}`;

  const cached = summaryCache.get(cacheKey);
  if (cached) {
    return {
      ...cached,
      goodForPills: [...cached.goodForPills]
    };
  }
