...
/* ── Compile jargon mappings once at startup ── */
...
// Sort swap phrases by descending length to prevent sub-string matching issues
const sortedSwapKeys = Object.keys(SWAPS_MAP).sort((a, b) => b.length - a.length);

// Escape keys for RegExp inclusion, joining them to create a single-pass regex
const escapedKeys = sortedSwapKeys.map(key => key.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'));
const SWAPS_REGEX = new RegExp(`\\b(${escapedKeys.join("|")})\\b`, "gi");

/* ── Replace jargon with normal words (Single-pass RegExp match) ── */
function simplifyWords(text: string): string {
  const normalized = normalizeText(text);
  return normalized.replace(SWAPS_REGEX, (matched) => {
    const lower = matched.toLowerCase();
    return SWAPS_MAP[lower] || matched;
  });
}
...
/* FIFO cache to avoid redundant summary calculations */
const SUMMARY_CACHE = new Map<string, RepoSummary>();
const CACHE_LIMIT = 1000;

function getCacheKey(repo: RepoLike): string {
  // Try using title and plainEnglishDescription as a stable composite key
  return `${repo.title || ""}:${repo.plainEnglishDescription || ""}:${repo.language || ""}`;
}
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  manageCache();
  SUMMARY_CACHE.set(cacheKey, summaryResult);
