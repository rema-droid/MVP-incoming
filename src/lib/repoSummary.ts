...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  // Simple FIFO cache for performance
  const cacheKey = `${repo.title || ""}|${repo.plainEnglishDescription || ""}|${repo.language || ""}|${(repo.topics || []).slice().sort().join(",")}`;
  const cached = summaryCache.get(cacheKey);
  if (cached) {
    // Return a shallow clone to prevent external mutations from affecting the cache
