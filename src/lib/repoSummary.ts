...
function simplifyWords(text: string): string {
  ...
  for (let i = 0; i < JARGON_SWAPS.length; i++) {
    const [re, to] = JARGON_SWAPS[i];
    t = t.replace(re, to);
  }
  return t;
}
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (SUMMARY_MATCHERS.game.test(combined)) {
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  for (let i = 0; i < CATEGORY_MATCHERS.length; i++) {
    if (CATEGORY_MATCHERS[i].re.test(combined)) {
      result = CATEGORY_MATCHERS[i].label;
      break;
    }
  }
