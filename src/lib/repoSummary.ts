...
/* ── Pre-compiled jargon replacements ── */
...
/* ── Hoisted regex constants for classification ── */
...
const RE_CONTAINER = /(docker|container)/;
...
/* ── Hoisted regex constants for friendly category label ── */
const RE_FRIENDLY_AI = /(ai|llm|chat|gpt|assistant|agent)/;
const RE_FRIENDLY_WEBSITE = /(react|next|vue|web|website|browser)/;
const RE_FRIENDLY_BACKEND = /(api|server|backend)/;
const RE_FRIENDLY_DATA = /(data|analytics|chart)/;
const RE_FRIENDLY_GAME = /(game|play)/;
...
/* ── Cache for summarizeRepoForBeginners ── */
...
function manageCache(key: string, value: RepoSummary) {
  if (!cache.has(key)) {
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  manageCache(cacheKey, summaryResult);
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_FRIENDLY_AI.test(combined)) return "Smart helper";
  if (RE_FRIENDLY_WEBSITE.test(combined)) return "Website";
  if (RE_FRIENDLY_BACKEND.test(combined)) return "Behind-the-scenes worker";
  if (RE_FRIENDLY_DATA.test(combined)) return "Numbers and charts";
  if (RE_FRIENDLY_GAME.test(combined)) return "Fun stuff";
  if (RE_FRIENDLY_CLI.test(combined)) return "Text-based tool";
  if (RE_FRIENDLY_CREATIVE.test(combined)) return "Creative tool";
  if (RE_FRIENDLY_SECURITY.test(combined)) return "Safety and privacy";
  if (RE_FRIENDLY_INFRA.test(combined)) return "Setup helper";
