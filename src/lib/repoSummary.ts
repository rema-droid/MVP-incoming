...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (SUMMARY_RE.docker.test(combined)) {
  ...
  if (SUMMARY_RE.fun.test(combined)) {
  ...
  if (SUMMARY_CACHE.size >= MAX_CACHE_SIZE) {
    ...
    if (firstKey !== undefined) SUMMARY_CACHE.delete(firstKey);
  }
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  let result = "";
  if (CATEGORY_RE.ai.test(combined)) result = "Smart helper";
  else if (CATEGORY_RE.website.test(combined)) result = "Website";
  else if (CATEGORY_RE.backend.test(combined)) result = "Behind-the-scenes worker";
  else if (CATEGORY_RE.data.test(combined)) result = "Numbers and charts";
  else if (CATEGORY_RE.fun.test(combined)) result = "Fun stuff";
  else if (CATEGORY_RE.cli.test(combined)) result = "Text-based tool";
  else if (CATEGORY_RE.creative.test(combined)) result = "Creative tool";
  else if (CATEGORY_RE.security.test(combined)) result = "Safety and privacy";
  else if (CATEGORY_RE.infra.test(combined)) result = "Setup helper";
  else result = "Community tool";
  if (CATEGORY_CACHE.size >= MAX_CACHE_SIZE) {
    const firstKey = CATEGORY_CACHE.keys().next().value;
    if (firstKey !== undefined) CATEGORY_CACHE.delete(firstKey);
  }
  CATEGORY_CACHE.set(key, result);
  return result;
}
