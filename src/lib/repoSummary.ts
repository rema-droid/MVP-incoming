...
/**
 * Cache for repository summaries to avoid expensive regex and string operations.
 * Using a simple FIFO strategy with a limit to prevent memory leaks.
 */
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  const plainEnglishDescription = repo.plainEnglishDescription || "";
  const language = repo.language || "";
  // Sort topics to ensure a stable cache key regardless of order
  const topics = [...(repo.topics || [])].sort().join(",");
  const cacheKey = `${title}|${plainEnglishDescription}|${language}|${topics}`;

  // Check cache to improve performance when rendering large lists of repos
  if (summaryCache.has(cacheKey)) {
    const cached = summaryCache.get(cacheKey)!;
    // Return a clone to ensure internal cache state remains immutable
