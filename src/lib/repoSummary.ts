...
function stripLangNames(text: string): string {
  return text.replace(LANG_RE, "").replace(MULTI_SPACE_RE, " ").trim();
}

/**
 * Performance optimization: Hoisting large static arrays and Regex literals
 * out of high-frequency functions prevents redundant re-allocation
 * and re-compilation overhead.
 */
...
// ── In-memory caches for high-traffic functions ────────────────────────────
const MAX_CACHE_SIZE = 1000;

const SUMMARY_CACHE = new Map<string, RepoSummary>();
const summaryCacheKeys: string[] = [];

const CATEGORY_CACHE = new Map<string, string>();
const categoryCacheKeys: string[] = [];

function setWithEviction<T>(cache: Map<string, T>, keys: string[], key: string, value: T) {
...
// ── Regex literals for classification ─────────────────────────────────────
const SUMMARY_RE = {
  website: /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/,
  backend: /(api|server|backend|graphql|microservice|rest)/,
  cli: /(cli|command tool|terminal|command-line|tool)/,
  python: /(python|django|flask|fastapi)/,
  rust: /(rust)/,
  data: /(data|analytics|pandas|spark|ml|model|dataset)/,
  security: /(auth|login|oauth|jwt|security|encryption)/,
  container: /(docker|container)/,
  ai: /(ai|llm|gpt|agent|neural|transformer|langchain|chat)/,
  game: /(game|play|fun)/,
  creative: /(image|video|audio|media|design|editor|creative)/,
};

/* ── Short summary (card-level) ── */
/**
 * Summarizes a repository for beginners.
 * Performance: Memoized with a FIFO cache (~0.003ms hot vs ~0.055ms cold).
 */
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  setWithEviction(SUMMARY_CACHE, summaryCacheKeys, cacheKey, result);
  return result;
}

const CATEGORY_RE = {
  ai: /(ai|llm|chat|gpt|assistant|agent)/,
  website: /(react|next|vue|web|website|browser)/,
  backend: /(api|server|backend)/,
  data: /(data|analytics|chart)/,
  game: /(game|play)/,
  cli: /(cli|terminal|command)/,
  creative: /(image|video|audio|media|design|creative)/,
  security: /(security|auth|encryption|privacy)/,
  container: /(docker|kubernetes|cloud|devops|infra)/,
};

/**
 * Friendly category label — never a programming language name.
 * Performance: Memoized with a FIFO cache (~0.001ms hot vs ~0.01ms cold).
 */
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CATEGORY_RE.ai.test(combined)) label = "Smart helper";
  else if (CATEGORY_RE.website.test(combined)) label = "Website";
  else if (CATEGORY_RE.backend.test(combined)) label = "Behind-the-scenes worker";
  else if (CATEGORY_RE.data.test(combined)) label = "Numbers and charts";
  else if (CATEGORY_RE.game.test(combined)) label = "Fun stuff";
  else if (CATEGORY_RE.cli.test(combined)) label = "Text-based tool";
  else if (CATEGORY_RE.creative.test(combined)) label = "Creative tool";
  else if (CATEGORY_RE.security.test(combined)) label = "Safety and privacy";
  else if (CATEGORY_RE.container.test(combined)) label = "Setup helper";

  setWithEviction(CATEGORY_CACHE, categoryCacheKeys, cacheKey, label);
  return label;
}
