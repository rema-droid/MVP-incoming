...
function getSummaryCacheKey(repo: RepoLike): string {
  const topicsStr = [...(repo.topics || [])].sort().join(",");
  return `${repo.title || ""}|${repo.plainEnglishDescription || ""}|${repo.language || ""}|${topicsStr}`;
}

/* ── Short summary (card-level) ── */
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  const cacheKey = getSummaryCacheKey(repo);
