...
/* ── Hoisted jargon-replacement rules ── */
...
function manageCache<T>(cache: Map<string, T>, key: string, value: T) {
...
function getCacheKey(repo: RepoLike): string {
  if (repo.id !== undefined) return String(repo.id);
  // Fallback to a composite key if no stable ID is provided
  return `${repo.title || ""}-${repo.plainEnglishDescription || ""}-${repo.language || ""}-${(repo.topics || []).join(",")}`;
}

/* ── Hoisted classification rules ── */
const TYPE_RULES: Array<{ re: RegExp; label: string; pill: string }> = [
  { re: /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/, label: "Website you can visit", pill: "Opens in your browser like any normal website" },
  { re: /(api|server|backend|graphql|microservice|rest)/, label: "Behind-the-scenes helper", pill: "Works quietly in the background so other apps can do their job" },
  { re: /(cli|command tool|terminal|command-line|tool)/, label: "Text-based helper", pill: "You tell it what to do by typing — no clicking needed" },
  { re: /(python|django|flask|fastapi)/, label: "", pill: "Popular with people who work with numbers and small handy tools" },
  { re: /(rust)/, label: "", pill: "Made to be really fast and really safe — like a sports car with extra seat belts" },
  { re: /(data|analytics|pandas|spark|ml|model|dataset)/, label: "Information explorer", pill: "Helps you look at numbers, charts, and patterns to understand things better" },
  { re: /(auth|login|oauth|jwt|security|encryption)/, label: "", pill: "Keeps your passwords, accounts, and private stuff safe" },
  { re: /(docker|container)/, label: "", pill: "Packages everything neatly so you can start with one tap — no fuss" },
  { re: /(ai|llm|gpt|agent|neural|transformer|langchain|chat)/, label: "Smart helper", pill: "Uses a computer brain to answer questions or do tasks for you" },
  { re: /(game|play|fun)/, label: "Fun and games", pill: "Something you play or have fun with — just for the joy of it" },
  { re: /(image|video|audio|media|design|editor|creative)/, label: "Creative tool", pill: "Helps you make, edit, or enjoy pictures, videos, or sounds" },
];
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  for (const rule of TYPE_RULES) {
    if (rule.re.test(combined)) {
      if (rule.label) typeHints.push(rule.label);
      if (rule.pill) pills.push(rule.pill);
    }
  }
  ...
  manageCache(SUMMARY_CACHE, key, result);
  return result;
}
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  manageCache(CATEGORY_CACHE, key, label);
  return label;
}
