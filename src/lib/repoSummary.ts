...
const TYPE_REGEXES = {
  ...
  behindScenes: /(api|server|backend|graphql|microservice|rest)/,
  textBased: /(cli|command tool|terminal|command-line|tool)/,
  python: /(python|django|flask|fastapi)/,
  rust: /(rust)/,
  infoExplorer: /(data|analytics|pandas|spark|ml|model|dataset)/,
  safety: /(auth|login|oauth|jwt|security|encryption)/,
  setup: /(docker|container)/,
  smartHelper: /(ai|llm|gpt|agent|neural|transformer|langchain|chat)/,
  fun: /(game|play|fun)/,
...
const CAT_REGEXES = {
  smartHelper: /(ai|llm|chat|gpt|assistant|agent)/,
  website: /(react|next|vue|web|website|browser)/,
  behindScenes: /(api|server|backend)/,
  infoExplorer: /(data|analytics|chart)/,
  fun: /(game|play)/,
  textBased: /(cli|terminal|command)/,
  creative: /(image|video|audio|media|design|creative)/,
  safety: /(security|auth|encryption|privacy)/,
  setup: /(docker|kubernetes|cloud|devops|infra)/,
};
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_REGEXES.smartHelper.test(combined)) return "Smart helper";
  if (CAT_REGEXES.website.test(combined)) return "Website";
  if (CAT_REGEXES.behindScenes.test(combined)) return "Behind-the-scenes worker";
  if (CAT_REGEXES.infoExplorer.test(combined)) return "Numbers and charts";
  if (CAT_REGEXES.fun.test(combined)) return "Fun stuff";
  if (CAT_REGEXES.textBased.test(combined)) return "Text-based tool";
  if (CAT_REGEXES.creative.test(combined)) return "Creative tool";
  if (CAT_REGEXES.safety.test(combined)) return "Safety and privacy";
  if (CAT_REGEXES.setup.test(combined)) return "Setup helper";
