...
/* ── Pre-compiled jargon maps and unified case-insensitive regular expression ── */
...
// Sort keys by length in descending order to match "open source" before "open"
const sortedKeys = Object.keys(SWAPS_MAP).sort((a, b) => b.length - a.length);

// Escape keys for RegExp
const escapedKeys = sortedKeys.map(key => key.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'));

// Build pattern with word boundaries
const patternString = '\\b(' + escapedKeys.join('|') + ')\\b';
const unifiedRegex = new RegExp(patternString, 'gi');

// Case sensitive patterns for ML/AI to maintain the exact behavior of original regexes
const caseSensitiveMap: Record<string, string> = {
  "ML": "machine learning (teaching a computer to learn patterns)",
  "AI": "a smart helper that can think and learn",
};
const caseSensitiveRegex = /\b(ML|AI)\b/g;

/* ── Replace jargon with normal words using O(1) single-pass regex replace ── */
function simplifyWords(text: string): string {
  let t = normalizeText(text);
  t = t.replace(unifiedRegex, (match) => {
    const key = match.toLowerCase();
    return SWAPS_MAP[key] || match;
  });
  t = t.replace(caseSensitiveRegex, (match) => {
    return caseSensitiveMap[match] || match;
  });
  return t;
}
