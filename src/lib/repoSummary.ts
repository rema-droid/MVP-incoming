...
/* ── PERF: Hoist jargon swaps to module level to avoid re-allocation ── */
...
/* ── PERF: Cache helpers to prevent memory leaks and speed up computation ── */
...
function getSummarizeCacheKey(repo: RepoLike): string {
  ...
  // Fallback for repos without an ID
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  const cached = SUMMARIZE_CACHE.get(cacheKey) as RepoSummary | undefined;
  if (cached) {
    // Return shallow clone to prevent external mutation of cached pills
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  const cacheKey = getSummarizeCacheKey(repo);
  const cached = CATEGORY_CACHE.get(cacheKey) as string | undefined;
