...
/* ── Hoisted Regexes for summarization and categories ── */
...
/* ── Hoisted Regexes for category labels ── */
const CAT_AI = /(ai|llm|chat|gpt|assistant|agent)/;
const CAT_REACT = /(react|next|vue|web|website|browser)/;
const CAT_API = /(api|server|backend)/;
...
const CAT_DOCKER = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_AI.test(combined)) return "Smart helper";
  if (CAT_REACT.test(combined)) return "Website";
  if (CAT_API.test(combined)) return "Behind-the-scenes worker";
  if (CAT_DATA.test(combined)) return "Numbers and charts";
  if (CAT_GAME.test(combined)) return "Fun stuff";
  if (CAT_CLI.test(combined)) return "Text-based tool";
  if (CAT_CREATIVE.test(combined)) return "Creative tool";
  if (CAT_SECURITY.test(combined)) return "Safety and privacy";
  if (CAT_DOCKER.test(combined)) return "Setup helper";
