...
const RE_WEB = /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/;
...
function manageCache<K, V>(cache: Map<K, V>, max: number) {
  if (cache.size > max) {
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  manageCache(SUMMARY_CACHE, MAX_CACHE_SIZE);

  return result;
}

/* Category Label Regexes */
