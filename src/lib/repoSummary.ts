...
function stripLangNames(text: string): string {
  return text.replace(LANG_RE, "").replace(RE_MULTI_SPACE, " ").trim();
}
...
function enforceCacheLimit(cache: Map<string, unknown>, limit: number) {
  if (cache.size >= limit) {
...
const RE_CAT_WEB = /(next|react|vue|angular|svelte|tailwind|css|ui|frontend)/;
const RE_CAT_SERVER = /(api|server|backend|graphql|microservice|rest)/;
const RE_CAT_CLI = /(cli|command tool|terminal|command-line|tool)/;
const RE_CAT_PY = /(python|django|flask|fastapi)/;
const RE_CAT_RUST = /(rust)/;
const RE_CAT_DATA = /(data|analytics|pandas|spark|ml|model|dataset)/;
const RE_CAT_AUTH = /(auth|login|oauth|jwt|security|encryption)/;
const RE_CAT_DOCKER = /(docker|container)/;
const RE_CAT_AI = /(ai|llm|gpt|agent|neural|transformer|langchain|chat)/;
const RE_CAT_GAME = /(game|play|fun)/;
const RE_CAT_MEDIA = /(image|video|audio|media|design|editor|creative)/;

const RE_TYPE_WEB = /website/i;
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  const cacheKey = JSON.stringify(repo);
  ...
  enforceCacheLimit(SUMMARY_CACHE, CACHE_LIMIT);
  SUMMARY_CACHE.set(cacheKey, result);

  return result;
}

const RE_LABEL_AI = /(ai|llm|chat|gpt|assistant|agent)/;
const RE_LABEL_WEB = /(react|next|vue|web|website|browser)/;
const RE_LABEL_SERVER = /(api|server|backend)/;
const RE_LABEL_DATA = /(data|analytics|chart)/;
const RE_LABEL_GAME = /(game|play)/;
const RE_LABEL_CLI = /(cli|terminal|command)/;
const RE_LABEL_MEDIA = /(image|video|audio|media|design|creative)/;
const RE_LABEL_SAFETY = /(security|auth|encryption|privacy)/;
const RE_LABEL_SETUP = /(docker|kubernetes|cloud|devops|infra)/;
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_LABEL_AI.test(combined)) result = "Smart helper";
  else if (RE_LABEL_WEB.test(combined)) result = "Website";
  else if (RE_LABEL_SERVER.test(combined)) result = "Behind-the-scenes worker";
  else if (RE_LABEL_DATA.test(combined)) result = "Numbers and charts";
  else if (RE_LABEL_GAME.test(combined)) result = "Fun stuff";
  else if (RE_LABEL_CLI.test(combined)) result = "Text-based tool";
  else if (RE_LABEL_MEDIA.test(combined)) result = "Creative tool";
  else if (RE_LABEL_SAFETY.test(combined)) result = "Safety and privacy";
  else if (RE_LABEL_SETUP.test(combined)) result = "Setup helper";

  enforceCacheLimit(CATEGORY_CACHE, CACHE_LIMIT);
  CATEGORY_CACHE.set(cacheKey, result);
  return result;
}
