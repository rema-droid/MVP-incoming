...
// PERF: Module-level constant to avoid redundant array allocations and regex compilations
...
function simplifyWords(text: string): string {
  ...
  for (const [re, to] of JARGON_SWAPS) {
    t = t.replace(re, to);
  }
  return t;
}

// PERF: Size-limited FIFO caches for expensive summary functions
...
function getSummarizeCacheKey(repo: RepoLike): string {
  // If id is present, it's the most reliable key. We use id: 0 as a valid ID check.
  if (repo.id !== undefined) return `id:${repo.id}`;
  // Fallback to a composite key that includes all inputs that affect the output
  return `key:${repo.title || ""}|${repo.language || ""}|${(repo.topics || []).join(",")}|${repo.stars || 0}|${repo.plainEnglishDescription || ""}`;
}
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  const key = getSummarizeCacheKey(repo);
  const cached = SUMMARIZE_CACHE.get(key);
  if (cached) {
    // PERF: Return a shallow clone of the object and a fresh copy of the array
    // to prevent callers from accidentally poisoning the cache via mutation.
  ...
  if (SUMMARIZE_CACHE.size >= MAX_CACHE_SIZE) {
    const firstKey = SUMMARIZE_CACHE.keys().next().value;
    if (firstKey !== undefined) SUMMARIZE_CACHE.delete(firstKey);
  }
  SUMMARIZE_CACHE.set(key, result);
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (/(ai|llm|chat|gpt|assistant|agent)/.test(combined)) res = "Smart helper";
  else if (/(react|next|vue|web|website|browser)/.test(combined)) res = "Website";
  else if (/(api|server|backend)/.test(combined)) res = "Behind-the-scenes worker";
  else if (/(data|analytics|chart)/.test(combined)) res = "Numbers and charts";
  else if (/(game|play)/.test(combined)) res = "Fun stuff";
  else if (/(cli|terminal|command)/.test(combined)) res = "Text-based tool";
  else if (/(image|video|audio|media|design|creative)/.test(combined)) res = "Creative tool";
  else if (/(security|auth|encryption|privacy)/.test(combined)) res = "Safety and privacy";
  else if (/(docker|kubernetes|cloud|devops|infra)/.test(combined)) res = "Setup helper";
