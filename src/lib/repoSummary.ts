...
/* ── Hoisted Jargon Swaps to avoid re-allocation ── */
...
/* ── Hoisted Regexes for common categories and types ── */
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (cached) return { ...cached, goodForPills: [...cached.goodForPills] }; // Clone to avoid mutation side-effects
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_AI.test(combined)) label = "Smart helper";
  else if (RE_WEBSITE.test(combined)) label = "Website";
  else if (RE_BACKEND.test(combined)) label = "Behind-the-scenes worker";
  else if (RE_DATA.test(combined)) label = "Numbers and charts";
  else if (RE_GAME.test(combined)) label = "Fun stuff";
  else if (RE_CLI.test(combined)) label = "Text-based tool";
  else if (RE_CREATIVE.test(combined)) label = "Creative tool";
  else if (RE_SECURITY.test(combined)) label = "Safety and privacy";
