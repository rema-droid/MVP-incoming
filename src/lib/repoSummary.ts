...
/* ── Replace jargon with normal words (Optimized single-pass) ── */
...
// Case-sensitive exact acronym keys (e.g. AI, ML)
const EXACT_CASE_MAP: Record<string, string> = {
  "AI": "a smart helper that can think and learn",
  "ML": "machine learning (teaching a computer to learn patterns)",
};

const EXACT_CASE_REGEX = /\b(AI|ML)\b/g;

// Case-insensitive key replacements
const SWAPS_KEYS = Object.keys(SWAPS_MAP)
  .map((k) => k.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&"))
  .sort((a, b) => b.length - a.length);

const SWAPS_REGEX = new RegExp(`\\b(${SWAPS_KEYS.join("|")})\\b`, "gi");

function simplifyWords(text: string): string {
  const normalized = normalizeText(text);
  const withAcronyms = normalized.replace(EXACT_CASE_REGEX, (match) => EXACT_CASE_MAP[match] || match);
  return withAcronyms.replace(SWAPS_REGEX, (match) => SWAPS_MAP[match.toLowerCase()] || match);
}
