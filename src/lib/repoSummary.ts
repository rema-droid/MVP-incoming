...
const repoSummaryCache = new Map<string, RepoSummary>();
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  const cached = repoSummaryCache.get(cacheKey);
  if (cached) {
    return cached;
  }
  ...
  if (repoSummaryCache.size < 1000) {
    repoSummaryCache.set(cacheKey, result);
  }

  return result;
}

const friendlyCategoryCache = new Map<string, string>();
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  const cached = friendlyCategoryCache.get(combined);
  if (cached) {
    return cached;
  }
  ...
  if (friendlyCategoryCache.size < 1000) {
    friendlyCategoryCache.set(combined, result);
  }
  return result;
}
