...
/* ── Pre-compiled regexes for classification ── */
...
const RE_GAMES = /(game|play|fun)/;
const RE_CREATIVE = /(image|video|audio|media|design|editor|creative)/;

const RE_AI_HINT = /(ai|llm|chat|gpt|assistant|agent)/;
const RE_WEB_HINT = /(react|next|vue|web|website|browser)/;
const RE_BACKEND_HINT = /(api|server|backend)/;
const RE_DATA_HINT = /(data|analytics|chart)/;
const RE_GAMES_HINT = /(game|play)/;
const RE_CLI_HINT = /(cli|terminal|command)/;
const RE_CREATIVE_HINT = /(image|video|audio|media|design|creative)/;
const RE_SECURITY_HINT = /(security|auth|encryption|privacy)/;
const RE_INFRA_HINT = /(docker|kubernetes|cloud|devops|infra)/;

/* ── Pre-compiled regexes for deep description logic ── */
const RE_TYPE_WEBSITE = /website/i;
const RE_TYPE_BEHIND = /behind/i;
const RE_TYPE_TEXT_BASED = /text-based/i;
const RE_TYPE_SMART_HELPER = /smart helper/i;
const RE_TYPE_INFORMATION = /numbers and charts|information explorer/i;
const RE_TYPE_CREATIVE = /creative/i;
const RE_TYPE_FUN = /fun/i;

/* ── Summary Cache (1000 items FIFO) ── */
const SUMMARY_CACHE = new Map<string, RepoSummary>();
const MAX_CACHE_SIZE = 1000;

function manageCache<T>(cache: Map<string, T>, key: string, value: T, limit: number) {
...
function getCacheKey(repo: RepoLike): string {
  ...
  // Composite key for repos without IDs
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  } else if (RE_TYPE_TEXT_BASED.test(typeLabel)) {
    useSentence =
  ...
  manageCache(SUMMARY_CACHE, cacheKey, result, MAX_CACHE_SIZE);
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (RE_AI_HINT.test(combined)) return "Smart helper";
  if (RE_WEB_HINT.test(combined)) return "Website";
  if (RE_BACKEND_HINT.test(combined)) return "Behind-the-scenes worker";
  if (RE_DATA_HINT.test(combined)) return "Numbers and charts";
  if (RE_GAMES_HINT.test(combined)) return "Fun stuff";
  if (RE_CLI_HINT.test(combined)) return "Text-based tool";
  if (RE_CREATIVE_HINT.test(combined)) return "Creative tool";
  if (RE_SECURITY_HINT.test(combined)) return "Safety and privacy";
  if (RE_INFRA_HINT.test(combined)) return "Setup helper";
