...
function setWithEviction<K, V>(cache: Map<K, V>, keys: K[], key: K, value: V) {
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  setWithEviction(SUMMARY_CACHE, summaryKeys, key, result);
  return result;
}
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (CAT_RES.AI.test(combined)) result = "Smart helper";
  else if (CAT_RES.WEB.test(combined)) result = "Website";
  else if (CAT_RES.BACKEND.test(combined)) result = "Behind-the-scenes worker";
  else if (CAT_RES.DATA.test(combined)) result = "Numbers and charts";
  else if (CAT_RES.FUN.test(combined)) result = "Fun stuff";
  else if (CAT_RES.CLI.test(combined)) result = "Text-based tool";
  else if (CAT_RES.CREATIVE.test(combined)) result = "Creative tool";
  else if (CAT_RES.SECURITY.test(combined)) result = "Safety and privacy";
  else if (CAT_RES.INFRA.test(combined)) result = "Setup helper";

  setWithEviction(CATEGORY_CACHE, categoryKeys, key, result);
  return result;
}
