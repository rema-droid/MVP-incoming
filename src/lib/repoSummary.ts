...
function simplifyWords(text: string): string {
  ...
  // Optimization: Moved SWAPS array outside to prevent re-allocation on every call
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (summaryCache.size >= MAX_CACHE_SIZE) summaryCache.clear();
...
export function friendlyCategoryLabel(repo: RepoLike): string {
  ...
  if (categoryCache.size >= MAX_CACHE_SIZE) categoryCache.clear();
