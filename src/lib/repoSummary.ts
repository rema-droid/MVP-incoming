...
/* ── PERF: Hoist constant regexes to module level to avoid re-allocation ── */
...
/* ── PERF: Memoization caches to avoid repeated heavy computation ── */
...
function getSummarizeCacheKey(repo: RepoLike): string {
  ...
  // Fallback composite key if ID is missing
...
export function summarizeRepoForBeginners(repo: RepoLike): RepoSummary {
  ...
  if (cached) {
    // Return a shallow clone of the cached summary to prevent mutations
    // if someone modifies the goodForPills array.
  ...
  SUMMARIZE_CACHE.set(cacheKey, summary);
