...
// Memoized card component to avoid re-generating SVG backdrops, summary text, and category labels on parent re-renders
const NewsTickerCard = React.memo(function NewsTickerCard({ repo, idx, mode }: NewsTickerCardProps) {
  ...
  const toneClass =
    idx % 3 === 0
      ...
      : idx % 3 === 1
        ? "from-violet-400/25"
        : "from-emerald-400/25";
  const categoryLabel = friendlyCategoryLabel(repo);
...
function NewsTicker({ repos }: NewsTickerProps) {
  ...
  const desktopRepos = useMemo(() => (repos ? repos.slice(0, 3) : []), [repos]);
  ...
  return (
    <div className="relative w-full mb-4">
      ...
      <div className="relative overflow-visible pb-2">
        ...
        >
          {repos.map((repo, idx) => (
            <NewsTickerCard key={`mobile-${repo.id}`} repo={repo} idx={idx} mode="mobile" />
          ))}
        </div>
        <div className="hidden grid-cols-3 gap-5 md:grid">
          {desktopRepos.map((repo, idx) => (
            <NewsTickerCard key={`desktop-${repo.id}`} repo={repo} idx={idx} mode="desktop" />
          ))}
        </div>
      </div>
    </div>
...
// Wrap NewsTicker in React.memo to prevent re-renders when parent state updates without changing repos prop
