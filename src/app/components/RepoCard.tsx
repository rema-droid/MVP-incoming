...
// FIFO cache for generated backdrop SVGs to avoid expensive re-encoding
...
export function getRepoBackdrop(repo: Repo) {
  // Use a copy to sort topics to avoid in-place mutation of props
  const cacheKey = `${repo.title}|${repo.language || ""}|${[...(repo.topics || [])].sort().join(",")}|${repo.owner || ""}`;
