...
function RepoCard({ repo, showPrice = false, onRun, variant = "list" }: RepoCardProps) {
  ...
  // summary is cached at the library level, but we useMemo here to avoid
  // redundant cache key generation and lookup on component re-renders.
  const summary = useMemo(() => summarizeRepoForBeginners(repo), [repo]);

  if (variant === "widget") {
    // These heavy computations are only needed for the widget variant.
    // Since the component is wrapped in React.memo, these will only re-run
    // when props (repo) change, making useMemo redundant inside this block.
