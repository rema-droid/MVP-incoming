...
/**
 * BOLT OPTIMIZATION: Cache generated SVG backgrounds.
 * SVG generation and URI encoding are expensive during large list renders.
 * Benchmark: Reduces average per-call latency significantly.
 */
const backdropCache = new Map<string, string>();
const BACKDROP_CACHE_LIMIT = 1000;

/* Backdrop SVG for widget cards — no external images, just a beautiful gradient */
export function getRepoBackdrop(repo: Repo) {
  const cacheKey = `${repo.id}|${repo.language}|${(repo.topics || []).join(",")}|${repo.title}|${repo.owner}`;
