    ...
    <aside className="hidden lg:flex w-[260px] flex-col border-r border-white/5 bg-[#031d24] h-[100dvh]">
      ...
      <div className="p-4 pt-4">
        <form onSubmit={onSearchSubmit} className="relative w-full">
          <label htmlFor="sidebar-search" className="sr-only">Search repositories</label>
          ...
          <input
            ...
            placeholder="Search"
            className="w-full rounded-md border border-white/10 bg-black/20 py-1.5 pl-9 pr-10 text-sm text-white placeholder-zinc-500 shadow-inner outline-none transition-all focus:border-blue-500/50 focus-visible:ring-blue-500/50 focus:bg-black/40 focus:ring-2 focus:ring-blue-500/20"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => onSearchChange && onSearchChange("")}
              className="absolute right-3 top-1/2 -translate-y-1/2 z-10 text-zinc-400 hover:text-white transition-colors"
          ...
          return (
            <div key={item.id} className="w-full">
              ...
              <button
                ...
                className={`group flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors focus-visible:ring-blue-500/50 outline-none ${
                  activeTab === item.id
