...
export default function Home() {
  ...
  const pageTitle = useMemo(() => isInSearchMode
    ...
    : "Recently Viewed", [activeTab, isInSearchMode]);

  const heroRepos = useMemo(() => !isInSearchMode && activeTab === "discover" ? feedRepos.slice(0, 8) : [], [activeTab, feedRepos, isInSearchMode]);
  const listRepos = useMemo(() => activeTab === "discover" && !isInSearchMode ? feedRepos.slice(8) : displayRepos, [activeTab, displayRepos, feedRepos, isInSearchMode]);
  const visibleRepos = useMemo(() => showAllRepos ? listRepos : listRepos.slice(0, 32), [listRepos, showAllRepos]);
