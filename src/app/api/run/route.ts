...
export async function POST(request: Request) {
  try {
    ...
    // Security: Validate repository URL to ensure it uses HTTPS
    // This prevents local file access or protocol smuggling.
    if (!repo.url.startsWith('https://')) {
      return NextResponse.json({ error: 'Invalid repository URL. Only HTTPS is allowed.' }, { status: 400 });
    }
