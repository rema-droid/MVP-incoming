...
export async function POST(request: Request) {
  try {
    ...
    // Security check: only allow https URLs to prevent local file access or protocol smuggling
    if (!repo.url.startsWith('https://')) {
      return NextResponse.json({ error: 'Invalid repository URL: only https:// protocol is allowed' }, { status: 400 });
    }
