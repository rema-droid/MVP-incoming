...
async function proxy(request: Request, context: { params: Promise<{ jobId: string; path: string[] }> }) {
  ...
  // Security: Strip sensitive headers from the incoming request to the upstream service
  ...
  // Security: Prevent the upstream service from setting cookies on the client
